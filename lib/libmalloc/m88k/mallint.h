...
#if m88k || clipper || pdp11
	struct header *dummy;	   /* pad to a multiple of 8 bytes */
#endif
};
/*
...
#if m88k
	int pad;		/* pad to a multiple of 8 bytes */
#endif
	char byte[sizeof(int)];	    /* There is no telling how big this
					   field really is.  */
};
/* 
...
#endif
	char space[sizeof(int)];/* start of space to allocate.
...
#endif
#endif
#define MINBLKSZ sizeof(struct header)	/* min. block size must as big as
				   HEADSZ */
#if m88k || ns32000 || clipper
#define BLOCKSZ 4096		/* memory is gotten from sbrk in
