 ...
 */
#define	_GetshLE(ip)	(* (short *) ip)
#define	_GetshBE(ip)	((short) (*((unsigned char *) ip) | (*(ip+1) << 8)))

#if	vax || pdp11 || clipper || ns32000
#define	_Getsh(ip)	GetshLE(ip)
#endif	/* vax || pdp11 || clipper || ns32000 */
/*
 * The following macro is partly due to Mike Laman, laman@sdcsvax
 *	NCR @ Torrey Pines.		- Tony Hansen
 */
#if	u3b || u3b15 || u3b2 || m68000 || m88k
#define	_Getsh(ip)	GetshBE(ip)
#endif	/* u3b || u3b15 || u3b2 || m68000 || m88k */
...
out_err:
    ...
    */
     ...
     */
    /* if we got the window size, and we are not forced to use LINES and
     * COLUMNS (USETIOCGWINSZ not in env) skip the following 
     */
#ifdef	TIOCGWINSZ
    if (gotsize == 0 || getenv("USETIOCGWINSZ") == NULL)
#endif
    {
