	...
	int flags, mode;
{
	...
	_strcpy(db->dbm_pagbuf, file);
	_strcat(db->dbm_pagbuf, ".pag");
	db->dbm_pagf = _open(db->dbm_pagbuf, flags, mode);
	if (db->dbm_pagf < 0)
		goto bad;
	_strcpy(db->dbm_pagbuf, file);
	_strcat(db->dbm_pagbuf, ".dir");
	...
	datum key;
{
	...
	(void) _lseek(db->dbm_pagf, db->dbm_blkno*PBLKSIZ, L_SET);
	if (_write(db->dbm_pagf, db->dbm_pagbuf, PBLKSIZ) != PBLKSIZ) {
	err:
...
__dbm_store(db, key, dat, replace)
	register DBM *db;
	datum key, dat;
	int replace;
{
...
split:
	...
	for (i=0;;) {
		...
		if (dcalchash(item) & (db->dbm_hmask+1)) {
			...
			if (item1.dptr == NULL) {
				_fprintf(stderr, "ndbm: split not paired\n");
				db->dbm_flags |= _DBM_IOERR;
				break;
			}
	...
	(void) _lseek(db->dbm_pagf, (db->dbm_blkno+db->dbm_hmask+1)*PBLKSIZ, L_SET);
	if (_write(db->dbm_pagf, ovfbuf, PBLKSIZ) != PBLKSIZ) {
		db->dbm_flags |= _DBM_IOERR;
		return (-1);
	}
	setbit(db);
	...
	register DBM *db;
{
	...
	for (;;) {
		if (db->dbm_blkptr != db->dbm_pagbno) {
			...
			(void) _lseek(db->dbm_pagf, db->dbm_blkptr*PBLKSIZ, L_SET);
	...
	register DBM *db;
{
	...
	if (b != db->dbm_dirbno) {
		db->dbm_dirbno = b;
		(void) _lseek(db->dbm_dirf, (long)b*DBLKSIZ, L_SET);
	...
	register DBM *db;
{
	...
	db->dbm_dirbno = b;
	(void) _lseek(db->dbm_dirf, (long)b*DBLKSIZ, L_SET);
	if (_write(db->dbm_dirf, db->dbm_dirbuf, DBLKSIZ) != DBLKSIZ)
		db->dbm_flags |= _DBM_IOERR;
}
