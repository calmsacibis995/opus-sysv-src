...
#define locungetc(x)	(chcount-=1,_ungetc(x,iop))
...
va_list ap;
{
	...
	for ( ; ; ) 
	{
		...
		switch(ch)
		{
		 ...
		 case 's':
		 case '[':
			  size = string(stow,ch,len,tab,iop,&ap);
			  break;
                 case 'n':
			  if (size == 'h')
				*va_arg(ap, short *) = (short) chcount;
		          else if (size == 'l')
				*va_arg(ap, long *) = (long) chcount;
			  else
			  	*va_arg(ap, int *) = (int) chcount;
			  continue;
		 case 'i':
                 default:
			 size = number(stow, ch, len, size, iop, &ap);
			 break;
                 }
		   if (size) 
		          nmatch += stow;   
		   ...
		   break;
	        if ( ((inchar = locgetc()) == 'x') || (inchar == 'X') ) 
	        {
		       ...
		       * then LOOK ahead         */
                   ...
                   if ( (iop->_file == _openmax()) || (iop->_cnt != 0) )
		      lookahead = locgetc();
		   else
		   {
		      if ( _read(fileno(iop),np,1) == 1)
		         lookahead = *np;
                      else
		         lookahead = EOF;
                      chcount += 1;
