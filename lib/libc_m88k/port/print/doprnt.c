...
extern char *_fcvt(), *_ecvt(), *_memchr(), *_memcpy();
extern int _strlen(), _fwrite();
...
register unsigned char **ptrptr;
{
	if (iop->_file != _openmax()) {
		...
		iop->_ptr = *ptrptr;
		_bufsync(iop);
		(void) _fwrite(p, 1, n, iop);
		*ptrptr = iop->_ptr;
	} else
		*ptrptr = (unsigned char *) _memcpy((char *) *ptrptr, p, n) + n;
}
	static char _blanks[] = "                    ";
...
register FILE	*iop;
{
	...
	/* Output values from fcvt and _ecvt */
	int	decpt, sign;
...
#if XXm88k	/* CCF stdio_cleanup defined in cuexit -- wrong place */
	/* Routines for doing cleanup */
	extern int (*_stdio_cleanup)();	/* Found in exit() module */
	extern int _cleanup();

	_stdio_cleanup = _cleanup;		/* Insure cleanup gets done */
#endif
	/* initialize buffer pointer and buffer end pointer */
	{ register int fno = iop->_file;
	bufptr = iop->_ptr;
	bufferend = (fno == _openmax()) ?
	...
	for ( ; ; ) {
		...
		if (fcode == '\0') {  /* end of format; return */
			...
			if (bufptr + iop->_cnt > bufferend && /* in case of */
					iop->_file != _openmax()) /* interrupt */
				_bufsync(iop);	/* during last several lines */
			cnt = iop->_ptr - iop->_base;
			if (count < cnt)
				cnt = count;
			if (iop->_flag & (_IONBF | _IOLBF) &&
				    (iop->_flag & _IONBF ||
				     _memchr((char *)(bufptr-cnt), '\n', cnt) != NULL))
				(void) _xflsbuf(iop);
	...
	charswitch:
		...
		case 'e':
			...
			bp = _ecvt(dval, min(prec + 1, MAXECVT), &decpt, &sign);
...
#endif
			/* Do the conversion */
			bp = _fcvt(dval, min(prec, MAXFCVT), &decpt, &sign);
		...
		case 'g':
			...
			/* Do the conversion */
			bp = _ecvt(dval, min(prec, MAXECVT), &decpt, &sign);
			if (dval == 0)
				decpt = 1;
