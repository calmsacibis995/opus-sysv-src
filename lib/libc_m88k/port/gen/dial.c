...
jmp_buf _Sjbuf;	/* needed by uucp routines */
static int
...
static int	Ifn;
static int	Debug = 0;
static int	Uid, Euid;		/* user-id and effective-uid */
static char	*P_PARITY = "Parity option error\r\n";
static char	Progname[NAMESIZE];
static char	Pchar;
static char	Rmtname[MAXFULLNAME];
static char	RemSpool[MAXFULLNAME];	/* spool subdirectory for remote system */
static char	User[MAXFULLNAME];
static char	Uucp[NAMESIZE];
static char	Loginuser[NAMESIZE];
static char	Myname[MAXBASENAME+1];
static char	Wrkdir[MAXFULLNAME];
static char	Logfile[MAXFULLNAME];
static char	*Spool = SPOOL;
static char	*Pubdir = PUBDIR;
static char	**Env;

static long	Retrytime = 0;
static struct	nstat Nstat;
static char	Dc[15];			/* line name				*/
static int	Seqn;			/* sequence #				*/
static int	Role;
static char	*Bnptr;			/* used when BASENAME macro is expanded */
static char	Jobid[NAMESIZE] = "";	/* Jobid of current C. file */
static int	Uerror;			/* global error code */

static int	Verbose = 0;	/* for cu and ct only */

/* used for READANY and READSOME macros */
static struct stat __s_;

/* messages */
static char	*Ct_OPEN =	"CAN'T OPEN";
static char	*Ct_WRITE =	"CAN'T WRITE";
static char	*Ct_READ =	"CAN'T READ";
static char	*Ct_CREATE =	"CAN'T CREATE";
static char	*Ct_ALLOCATE =	"CAN'T ALLOCATE";
static char	*Ct_LOCK =	"CAN'T LOCK";
static char	*Ct_STAT =	"CAN'T STAT";
static char	*Ct_CHOWN =	"CAN'T CHOWN";
static char	*Ct_CHMOD =	"CAN'T CHMOD";
static char	*Ct_LINK =	"CAN'T LINK";
static char	*Ct_CHDIR =	"CAN'T CHDIR";
static char	*Ct_UNLINK =	"CAN'T UNLINK";
static char	*Wr_ROLE =	"WRONG ROLE";
static char	*Ct_CORRUPT =	"CAN'T MOVE TO CORRUPTDIR";
static char	*Ct_CLOSE =	"CAN'T CLOSE";
static char	*Ct_FORK =	"CAN'T FORK";
static char	*Fl_EXISTS =	"FILE EXISTS";
...
static char *Myline = NULL;	/* flag to force the requested line to be used
			   by rddev() in conn.c */

_dial(call)
CALL call;
...
extern void	setservice();
	...
	if (call.speed <= 0) _strcpy(speed,"Any");
	else _sprintf(speed,"%d",call.speed);
/* Determine whether contents of telno is a phone number or a system name */
	if(_strlen(call.telno) != _strspn(call.telno,"0123456789=-*#"))
		systemname++;
/* If dial() was given a system name use "conn()" */
	...
	(void)savline();
	if ((call.attr) && _ioctl(rlfd, TCSETA, call.attr) < 0) {
		_perror("stty for remote");
		return(L_PROB);
	}
	Euid = _geteuid();
	if(_setuid(_getuid()) && _setgid(_getgid()) < 0)
		_undial(rlfd);
	return(rlfd);
}
...
static cleanup(code,Cn) 	/*this is executed only in the parent process*/
...
#define PREFIX(pre, str)	(_strncmp((pre), (str), _strlen(pre)) == SAME)
#define BASENAME(str, c) ((Bnptr = _strrchr((str), c)) ? (Bnptr + 1) : (str))
#define EQUALS(a,b)	((a != CNULL) && (b != CNULL) && (_strcmp((a),(b))==SAME))
#define EQUALSN(a,b,n)	((a != CNULL) && (b != CNULL) && (_strncmp((a),(b),(n))==SAME))
...
#ifdef ATTSV
#define index _strchr
#define rindex strrchr 
#define vfork fork
#define ATTSVKILL
#define UNAME
#else
#define _strchr index
...
extern char	*_strcat(), *_strncpy(), *_strrchr();
extern char	*_strchr(), *strpbrk(), *_strtok();
extern char	*index(), *rindex(), *getlogin(), *ttyname(), *_malloc();
extern char	*_calloc();
extern long	_times(), lseek(), atol();
extern time_t	time();
extern int	_strlen(), _strcmp(), _strncmp();
extern int	execle(), fork(), pipe(), _close(), _fcntl(), getopt();
 ...
 */
#define CDEBUG(l, f, s) if (Debug >= l) _fprintf(stderr, f, s)
#ifndef SMALL
#define DEBUG(l, f, s) if (Debug >= l) _fprintf(stderr, f, s)
#else
#define DEBUG(l, f, s)
#endif /* SMALL */
...
#ifdef STANDALONE
#define VERBOSE(f, s) if (Verbose > 0) _fprintf(stderr, f, s); else
#else
#define VERBOSE(f, s)
#endif
...
extern int	_read(), _write(), _ioctl();
int	usetup(), uteardown();
...
static
  struct Interface {
	...
	int	(*IN_teardown)();	/* teardown function, called after last
			...
			/* vanilla UNIX */
		{ "UNIX", _read, _write, _ioctl, usetup, uteardown },
#ifdef TLI
...
char	*label;
{
	...
	for ( i = 0;  Interface[i].IN_label;  ++i ) {
		if( !_strcmp( Interface[i].IN_label, label ) ) {
...
char *buf;
{
	...
	while (getdevline(buf, BUFSIZ)) {
		...
		if ( _strncmp(dev[D_LINE],"/dev/",5) == 0 ) {
			/* since cu (altconn()) strips off leading */
			/* "/dev/",  do the same here.  */
			_strcpy(dev[D_LINE], &(dev[D_LINE][5]) );
		}
...
/* For cu -- to force the requested line to be used */
		 ...
		 */
		if ((commap = _strchr(dev[D_TYPE], ',')) != (char *)NULL )
			*commap = '\0';
...
register char *flds[], *dev[];
{
	...
	if (dcf == -1) {
		 ...
		 */
		if ( *dev[D_LINE] != '/' ) {
			(void) _sprintf(dcname, "/dev/%s", dev[D_LINE]);
		} else {
			(void) _strcpy(dcname, dev[D_LINE] );
		}
		/* take care of the possible partial open fd */
		(void) _close(nullfd = _open("/", 0));
		if (setjmp(_Sjbuf)) {
			(void) _close(nullfd);
		...
		(void) _signal(SIGALRM, alarmtr);
		(void) _alarm(10);
		dcf = _open(dcname, O_RDWR);
		(void) _alarm(0);
		if (dcf < 0) {
...
#endif /* ATTSV */
		fixline(dcf, _atoi(fdig(dev[D_CLASS])), D_DIRECT);
	}
...
char *name;
{
	...
	if ( _strchr(name, '/') != NULL )
		return(0);
	(void) _sprintf(lname, "%s.%s", LOCKPRE, name);
...
int	fd;
{
	...
	while ( (lockrtn = _fcntl(fd, F_SETLK, &lck)) < 0 ) {
...
/* set line attributes associated with -h, -t, -e, and -o options */
	...
	ASSERT(_ioctl(tty, TCSETAW, &lv) >= 0,
...
restline()
{
	return(_ioctl(0, TCSETAW, &Savettyb));
}
...
int	dcf;
{
	...
	if (_ioctl(dcf, TCGETA, &ttbuf) != 0)
		return;
	if (!(ttbuf.c_cflag & HUPCL)) {
		ttbuf.c_cflag |= HUPCL;
		(void) _ioctl(dcf, TCSETAW, &ttbuf);
	}
}
...
uteardown( role, fdread, fdwrite )
{
	...
	if (fdread != -1) {
		...
		ttyn = ttyname(fdread);
		if (ttyn != NULL)
			_chmod(ttyn, Dev_mode);	/* can fail, but who cares? */
#endif /* DIAL */
		(void) _close(fdread);
		(void) _close(fdwrite);
	}
...
int	*fdwritep;
{
	...
	while ( getpop(strmod, sizeof(strmod), &optional) ) {
		DEBUG(5, 
			( optional ? "tssetup: optionally POPing %s\n"
				   : "tssetup: POPing %s\n" ),
			strmod);
		if ( _ioctl(*fdreadp, I_LOOK, onstream) == -1 ) {
			DEBUG(5, "tssetup: I_LOOK on fd %d failed", *fdreadp);
			return(FAIL);
		}
		if ( _strcmp(strmod, onstream) != SAME ) {
			if ( optional )
				continue;
			DEBUG(5, "tssetup: I_POP: %s not there\n", strmod);
			return(FAIL);
		}
		if ( _ioctl(*fdreadp, I_POP, 0) == -1 ) {
	...
	while ( getpush(strmod, sizeof(strmod)) ) {
		...
		if ( _ioctl(*fdreadp, I_PUSH, strmod) == -1 ) {
...
char *in, *out;
{
	...
	*s1 = NULLCHAR;
	...
	tpre[0] = NULLCHAR;
	fn = _fopen(DIALCODES, "r");
	if (fn != NULL) {
		while (_fgets(buf, BUFSIZ, fn)) {
			if ( _sscanf(buf, "%s%s", p, tpre) < 1)
				continue;
			if (EQUALS(p, pre))
				break;
			tpre[0] = NULLCHAR;
	...
	(void) _strcpy(out, tpre);
	(void) _strcat(out, npart);
	return;
}
...
char *s;
{
	...
	(void) _sprintf(ln, "%s.%s", LOCKPRE, s);
...
char *flds[], *dev[];
{
	...
	_strcpy(dialstring, dev[D_ARG]);

#ifndef STANDALONE
	if(*flds[F_CLASS] < '0' || *flds[F_CLASS] > '9')
		_sprintf(dialstring, "%s.%s", dev[D_ARG], flds[F_CLASS]);
#endif
	...
	DEBUG(4, ", %d)\n", _atoi(dev[D_CLASS]));
    	if ((fd = tdkdial(flds[F_PHONE], _atoi(dev[D_CLASS]))) >= 0)
...
char *flds[], *dev[];
{
	...
	dcr = _open(dcname, O_WRONLY|O_NDELAY);
	if (dcr < 0) {
		Uerror = SS_DIAL_FAILED;
	...
	sytfixline(dcr, _atoi(fdig(dev[D_CLASS])), D_DIRECT);
	(void) _sleep(2);
	DEBUG(4, "Calling Sytek unit %s\n", dev[D_ARG]);
	(void) _sprintf(command,"\r\rcall %s\r", dev[D_ARG]);
	ret = (*Write)(dcr, command, _strlen(command));
	(void) _sleep(1);
	...
	(void) _alarm(10);
	dcr2 = _open(dcname,O_RDWR);
	(void) _alarm(0);
	(void) _close(dcr);
	if (dcr2 < 0) {
		DEBUG(4, "OPEN 2 FAILED %s\n", dcname);
		Uerror = SS_DIAL_FAILED;
		(void) _close(nullfd);	/* kernel might think dc2 is open */
		delock(dev[D_LINE]);
		return(FAIL);
	}
	return(dcr2);
...
char *dev[];
{
	...
	if ( dev[D_LINE][0] != '/' ) {
		...
		_sprintf(devname, "/dev/%s", dev[D_LINE]);
	} else {
		/*	dev holds full path name of device	*/
		_strcpy(devname, dev[D_LINE]);
	}
...
char	*s;
{
	...
	if (0 < t_errno && t_errno < t_nerr) {
		...
		if ( t_errno == TSYSERR ) {
			DEBUG(5, "", _perror("tlicall: system error"));
		} else if ( t_errno == TLOOK ) {
			show_tlook(fd);
		}
	} else {
		_sprintf(fmt, "unknown tli error %d", t_errno);
		logent(s, fmt);
		_sprintf(fmt, "%s: unknown tli error %d", s, t_errno);
		DEBUG(5, fmt, 0);
		DEBUG(5, "", _perror(s));
	}
}
...
struct netbuf	*addr;
{
	...
	if (!addr) {
		if ((addr = (struct netbuf *)_malloc(sizeof(struct netbuf))) == NULL)
			return NULL;
		myadr = TRUE;
		addr->buf = NULL;
		addr->maxlen = 0;
...
int	type;
{
	...
	if ((*s == 0) || (end == s) || (!isspace(*end) && *end)) {
		_fprintf(stderr, "dobase: Illegal trailer on address string\n");
		buf[0] = '\0';
		return 0;
	}
	--end;
	...
	if (bp == 0) {
		_fprintf(stderr, "stoa: dobase: number to long\n");
		return 0;
	}
...
unsigned	len, max;
{
	if (max && dest && max < len) {		/* No room */
		_fprintf(stderr, "xfer: destination not long enough\n");
		return NULL;
	}
	if (!dest)
		if ((dest = _malloc(len)) == NULL) {
			_fprintf(stderr, "xfer: malloc failed\n");
			return NULL;
		}
...
char *flds[], *dev[];
{
	...
	(void) _sprintf(dnname, "/dev/%s", dev[D_CALLDEV]);
	(void) _sprintf(phone, "%s%s", dev[D_ARG]   , ACULAST);
	(void) _sprintf(dcname, "/dev/%s", dev[D_LINE]);
	...
	speed = _atoi(fdig(dev[D_CLASS]));
	dcf = open801(dcname, dnname, phone, speed);
	if (dcf >= 0) {
		fixline(dcf, speed, D_ACU);
		(void) _strcpy(Dc, dev[D_LINE]);	/* for later unlock() */
		VERBOSE("SUCCEEDED\n", 0);
	} else {
		delock(dev[D_LINE]);
		VERBOSE("FAILED\n", 0);
...
char *dcname, *dnname, *phone;
{
	...
	(void) _close(nullfd = _open("/dev/null", 0));	/* partial open hack */
	if (setjmp(_Sjbuf)) {
	...
	(void) _signal(SIGALRM, alarmtr);
	timelim = 5 * _strlen(phone);
	(void) _alarm(timelim < 30 ? 30 : timelim);
	if ((pid = fork()) == 0) {
		_sleep(2);
		nw = (*Write)(dnf, phone, lt = _strlen(phone));
		if (nw != lt) {
	...
	(void) _close(dnf);	/* no reason to keep the 801 open */
	if (lt != 0) {
		DEBUG(4, "Fork Stat %o\n", lt);
		(void) _close(dcf);
		Uerror = SS_DIAL_FAILED;
...
char *dcname, *dnname, *phone;
{
	...
	(void) _close(nullfd = _open("/", 0));	/* partial open hack */
	if (setjmp(_Sjbuf)) {
		DEBUG(4, "DN write %s\n", "timeout");
		(void) _close(dnf);
		(void) _close(dcf);
	...
	if ((dnf = _open(dnname, O_WRONLY)) < 0 ) {
		DEBUG(5, "can't open %s\n", dnname);
		Uerror = SS_CANT_ACCESS_DEVICE;
		return(FAIL);
	}
	...
	if (  (dcf = _open(dcname, O_RDWR | O_NDELAY)) < 0 ) {
		DEBUG(5, "can't open %s\n", dcname);
		Uerror = SS_CANT_ACCESS_DEVICE;
		return(FAIL);
	}
	...
	(void) _close(dnf);
	(void) _close(nullfd = _open("/", 0));	/* partial open hack */
	ret = _open(dcname, 2);  /* wait for carrier  */
	(void) _alarm(0);
	(void) _close(ret);	/* close 2nd modem open() */
	if (ret < 0) {		/* open() interrupted by alarm */
		DEBUG(4, "Line open %s\n", "failed");
		Uerror = SS_DIAL_FAILED;
		(void) _close(nullfd);		/* close partially opened modem */
		return(FAIL);
	}
...
char *flds[], *dev[];
{
	...
	(void) _signal(SIGPIPE, SIG_IGN);  /* watch out for broken ipc link...*/
	(void) _alarm(0);
	(void) _strcpy(Dc, "IPC");
	return(ret);
}
...
time_t atime;
{
	...
	if (pid[0] == '\0') {
		(void) _sprintf(pid, "%*d\n", SIZEOFPID, _getpid());
		(void) _sprintf(tempfile, "%s/LTMP.%d", X_LOCKDIR, _getpid());
	}
...
int fn;
{
	...
	if (EQUALS(str, "EOT")) {
		CDEBUG(5, "EOT\n", 0);
		(void) (*Write)(fn, EOTMSG, _strlen(EOTMSG));
		return;
	}
...
register char *name;
{
	...
	for (i = 0; i < Nlocks; i++) {
		...
		if (name == NULL || EQUALS(name, Lockfile[i])) {
			(void) _unlink(Lockfile[i]);
			(void) _free(Lockfile[i]);
			Lockfile[i] = NULL;
		}
	}
	return;
...
{	
	...
	fd=_creat(tempfile, 0444);
	if(fd < 0){
		(void) _sprintf(cb, "%s %s %d",tempfile, name, errno);
		logent("ULOCKC", cb);
		if((errno == EMFILE) || (errno == ENFILE))
			(void) _unlink(tempfile);
		return(-1);
	}
	(void) _write(fd, pid, SIZEOFPID+1);	/* +1 for '\n' */
	(void) _chmod(tempfile,0444);
	(void) _chown(tempfile, UUCPUID, UUCPGID);
	(void) _close(fd);
	if(_link(tempfile,name)<0){
		DEBUG(4, "%s: ", sys_errlist[errno]);
		DEBUG(4, "_link(%s, ", tempfile);
		DEBUG(4, "%s)\n", name);
		if(_unlink(tempfile)< 0){
			(void) _sprintf(cb, "ULK err %s %d", tempfile,  errno);
			logent("ULOCKLNK", cb);
		}
		return(-1);
	}
	if(_unlink(tempfile)<0){
		(void) _sprintf(cb, "%s %d",tempfile,errno);
		logent("ULOCKF", cb);
	}
	return(0);
}
...
register char *file;
{
	...
	ret = _read(fd, (char *) alpid, SIZEOFPID+1); /* +1 for '\n' */
	(void) _close(fd);
	if (ret != (SIZEOFPID+1)) {
...
char *name;
{
	...
	if (i >= Nlocks)
		i = Nlocks++;
	p = _calloc((unsigned) _strlen(name) + 1, sizeof (char));
...
unsigned n;
{
	...
	if (n > HZ) {
		...
		_sleep( (int) (((n)+HZ/2)/HZ) );
		return;
	}

	/* use timing loop for < 1 second */
	endtime = _times(&tbuf) + 3*n/4;	/* use 3/4 because of scheduler! */
	while (_times(&tbuf) < endtime) {
...
/* private variables */
void tokenize(), nameparse(), setfile(), setioctl(),
	scansys(), scancfg();
int namematch(), nextdialers(), nextsystems(), getline();
	...
	int type;
{
	...
	case ACCESS_SYSTEMS:
		return(_access(Systems[nsystems], R_OK));
	case ACCESS_DEVICES:
		return(_access(Devices[ndevices], R_OK));
	case ACCESS_DIALERS:
		return(_access(Dialers[ndialers], R_OK));
	...
	default:
		(void)_sprintf(errformat, "bad access type %d", type);
...
{	char *lptr, *lend;
	...
	lptr = line;
	while (_fgets(lptr, (line + BUFSIZ) - lptr, f) != NULL) {
		lend = lptr + _strlen(lptr);
		if (lend == lptr || lend[-1] != '\n')	
...
{	char *lend;
	...
	if (_strncmp(label, line, _strlen(label)) != SAME) {
	 ...
	 */
	while ((lend = _strchr(line, ':')) != NULL) {
		*lend = '\0';
		if (_strcmp(line, name) == SAME)
			return(TRUE);
		line = lend+1;
	}
	return(_strcmp(line, name) == SAME);
}
...
{	char *tok;
	...
	tokptr = tokens;
	while ((tok = _strtok((char *) NULL, " \t")) != NULL) {
		*tokptr++ = tok;
...
{	char **line, *equals;
	...
	for (line = tokens; (line - tokens) < NTOKENS && *line; line++) {
		...
		*equals = '\0';
		...
		if (*++equals == '\0')
			continue;
		if (_strcmp(*line, "systems") == SAME)
			setfile(Systems, equals);
		else if (_strcmp(*line, "devices") == SAME)
			setfile(Devices, equals);
		else if (_strcmp(*line, "dialers") == SAME)
			setfile(Dialers, equals);
		else if (_strcmp(*line, "pop") == SAME)
			setioctl(Pops, equals);
		else if (_strcmp(*line, "push") == SAME)
			setioctl(Pushes, equals);
		else {
			(void)_sprintf(errformat,"unrecognized label %s",*line);
...
{	char **tptr, *tok;
	...
	for (tok = _strtok(line, ":"); tok != NULL;
	tok = _strtok((char *) NULL, ":")) {
		expandpath[0] = '\0';
		if ( *tok != '/' )
			/* by default, file names are relative to SYSDIR */
			_sprintf(expandpath, "%s/", SYSDIR);
		_strcat(expandpath, tok);
	...
	char *buf;
{
	...
	if (fsystems == NULL)
		if (nextsystems() == FALSE)
			return(FALSE);
	for(;;) {
		if (_fgets(buf, len, fsystems) != NULL)
			return(TRUE);
		if (nextsystems() == FALSE)
			return(FALSE);
	}
...
nextsystems()
{
	...
	if (fsystems != NULL) {
		(void) _fclose(fsystems);
		nsystems++;
	} else {
		nsystems = 0;
	}
	for ( ; Systems[nsystems] != NULL; nsystems++)
		if ((fsystems = _fopen(Systems[nsystems], "r")) != NULL)
			return(TRUE);
	return(FALSE);
}
	...
	char *buf;
{
	...
	if (fdevices == NULL)
		if (nextdevices() == FALSE)
			return(FALSE);
	for(;;) {
		if (_fgets(buf, len, fdevices) != NULL)
			return(TRUE);
		if (nextdevices() == FALSE)
			return(FALSE);
	}
...
{
	if (fdevices != NULL) {
		(void) _fclose(fdevices);
		ndevices++;
	} else {
		ndevices = 0;
	}
	for ( ; Devices[ndevices] != NULL; ndevices++)
		if ((fdevices = _fopen(Devices[ndevices], "r")) != NULL)
			return(TRUE);
	return(FALSE);
}
	...
	char *buf;
{
	...
	if (fdialers == NULL)
		if (nextdialers() == FALSE)
			return(FALSE);
	for(;;) {
		if (_fgets(buf, len, fdialers) != NULL)
			return(TRUE);
		if (nextdialers() == FALSE)
			return(FALSE);
	}
...
nextdialers()
{
	...
	for ( ; Dialers[ndialers] != NULL; ndialers++)
		if ((fdialers = _fopen(Dialers[ndialers], "r")) != NULL)
			return(TRUE);
	return(FALSE);
}
	...
	int len, *optional;
{
	...
	slen = _strlen(Pops[npops]) - 1;
	if ( Pops[npops][0] == '('  && Pops[npops][slen] == ')' ) {
		*optional = 1;
		--slen;
		_strncpy(buf, &(Pops[npops++][1]), (slen <= len ? slen : len) );
	} else {
		*optional = 0;
		_strncpy(buf, Pops[npops++], len);
	}
	return(TRUE);
}
	...
	int len;
{
	if ( Pushes[0] == NULL || Pushes[npushes] == NULL )
		return(FALSE);
	_strncpy(buf, Pushes[npushes++], len);
	return(TRUE);
}
...
register int	amode;
{
	...
	amode <<= 3;
	if( _getegid() == s.st_gid  &&  (amode & s.st_mode) == amode )
		return  0;		/* access permitted by group mode */

	amode <<= 3;
	if( _geteuid() == s.st_uid  &&  (amode & s.st_mode) == amode )
