...
#    define COMPARE(A, B) (* hcompar)((A), (B))
     extern int (* hcompar)();
#else
#    define COMPARE(A, B) _strcmp((A), (B))
#endif
...
extern char *_malloc(), *_calloc(), *strcpy();
...
#endif
	new = (ENTRY *) _malloc(sizeof(ENTRY));
	if(new == NULL) {
	    fprintf(stderr, "Out of core \n");
	    exit(FAIL);
	}
	else {
	    new->key = _malloc((unsigned) strlen(line) + 1);
	    if(new->key == NULL) {
		fprintf(stderr, "Out of core \n");
		exit(FAIL);
	    }
	    strcpy(new->key, line);
	    new->data = _malloc(sizeof(int));
	    if(new->data == NULL) {
		fprintf(stderr, "Out of core \n");
		exit(FAIL);
	    }
...
int
_hcreate(size)		/* Create a hash table no smaller than size */
int size;		/* Minimum size for hash table */
{
    ...
    table = (TABELEM *) _calloc(length, sizeof(TABELEM));
    return(table != NULL);
}

void
_hdestroy()	/* Reset the module to its initial state */
{
    _free((POINTER) table);
#ifdef OPEN
    count = 0;
...
ENTRY
*_hsearch(item, action)	/* Find or insert the item into the table */
...
ENTRY
*_hsearch(item, action)	/* Chained search with sorted lists */
ENTRY item;		/* Item to be inserted or found */
ACTION action;		/* FIND or ENTER */
{
...
{
    NODE *p = (NODE *) _malloc(sizeof(NODE));
