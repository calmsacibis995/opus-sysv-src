	...
	while ((c = *format++) != '\0') {
		if (c == '%') {
			...
			case 'a':	/* Abbreviated weekday name */
				for(p = _cftime[STR.aSun + tm->tm_wday]; *p != '\0';p++)
					*cp++ = *p;
				continue;
			case 'A':	/* Weekday name */
				for(p =  _cftime[STR.Sun + tm->tm_wday]; *p != '\0';p++)
					*cp++ = *p;
				continue;
			case 'h':
			case 'b':	/* Abbreviated month name */
				for(p = _cftime[STR.aJan + tm->tm_mon];*p != '\0';p++)
					*cp++ = *p;
				continue;
			case 'B':	/* Month name */
				for(p = _cftime[STR.Jan + tm->tm_mon];*p != '\0'; p++)
					*cp++ = *p;
				continue;
			...
			case 'p':	/* AM or PM */
				if (tm->tm_hour >= 12) 
					for (p = _cftime[STR.PM]; *p != '\0'; p++)
						*cp++ = *p;
				else
					for (p = _cftime[STR.AM]; *p != '\0'; p++)
                                                *cp++ = *p;
				continue;
			...
			case 'x':	/* Localized date format */
				cp += ascftime(cp, _cftime[STR.Local_date], tm);
				continue;
			case 'X':	/* Localized time format */
				cp += ascftime(cp, _cftime[STR.Local_time], tm);
				continue;
...
setcftime()
{
	register char *p;
	register int j;
	char *mystr[sizeof(struct STR)], *file,  *q;
