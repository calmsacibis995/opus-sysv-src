...
#ifdef m88k
static struct header m_array[4];
static struct header *arena = &m_array[0];
static struct header *freeptr = &m_array[2];
#else
...
unsigned size;	    /* size to change to */
{
	...
	if (TESTSMAL(((struct lblk *)(ptr - MINHEAD))->header.holder))  {
		...
		if(ptr != newptr)  {
			(void)_memcpy(newptr,ptr,(int)size);
			_free(ptr);
		}
	}  else  {
		blk = (struct header *)(ptr - minhead);
		...
		if (cpysize >= trusize)  {
			...
			newptr = _malloc(size);
			if (newptr == NULL)  return NULL;
			(void)_memcpy(newptr,ptr,(int)cpysize);
			_free(ptr);
		}       
	}
	return newptr;
...
_mallinfo()
{
	...
	(void)_memset (&inf, 0, sizeof(struct mallinfo));
	if (freeptr->nextfree == GROUND)
	       return (inf);
