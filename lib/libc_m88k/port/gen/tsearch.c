...
NODE *
_tsearch(key, rootp, compar)	/* Find or insert key into search tree*/
...
int	(*compar)();		/* Comparison function */
{
	...
	q = (NODE *) _malloc(sizeof(NODE));	/* T5: Not found */
...
NODE *
_tdelete(key, rootp, compar)	/* Delete node with key key */
...
int	(*compar)();		/* Comparison function */
{
	...
	_free((POINTER) *rootp);		/* D4: Free node */
	*rootp = q;			/* Link parent to replacement */
	return (p);
}

void
_twalk(root, action)		/* Walk the nodes of a tree */
