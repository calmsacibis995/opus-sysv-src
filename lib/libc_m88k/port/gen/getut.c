...
struct utmp *_getutent()
{
	extern int fd;
	extern char utmpfile[];
	extern struct utmp ubuf;
	extern long loc_utmp,_lseek();
	extern int errno;
	register char *u;
	register int i;
	struct stat stbuf;
...
/* Save the location in the file where this entry was found. */
	loc_utmp = _lseek(fd,0L,1) - (long)(sizeof(struct utmp));
	return(&ubuf);
}
...
struct utmp *_getutline(entry)
register struct utmp *entry;
{
	extern struct utmp ubuf,*_getutent();
...
/* a pointer to it. */
		if (cur->ut_type != EMPTY && (cur->ut_type == LOGIN_PROCESS
		    || cur->ut_type == USER_PROCESS) && _strncmp(&entry->ut_line[0],
		    &cur->ut_line[0],sizeof(cur->ut_line)) == 0) return(cur);
	} while ((cur = _getutent()) != NULL);
...
/* Make sure file is writable */
	if ((fc=_fcntl(fd, F_GETFL, NULL)) == -1
	    || (fc & O_RDWR) != O_RDWR) {
		return((struct utmp *)NULL);
	}
...
/* utmp structures into the file incase the file is scribbled.) */
	...
	if (_getutid(&tmpbuf) == NULL) {
		...
		if (_getutid(&tmpbuf) == NULL) {
#ifdef	ERRDEBUG
			loc_utmp = _lseek(fd, 0L, 1);
			gdebug("Second getutid() failed.  fd: %d loc_utmp: %ld\n",fd,loc_utmp);
#endif
			_fcntl(fd, F_SETFL, fc | O_APPEND);
		} else {
			_lseek(fd, -(long)sizeof(struct utmp), 1);
		}
	} else {
		_lseek(fd, -(long)sizeof(struct utmp), 1);
	}
...
/* utmp file. */
	if (_write(fd,&tmpbuf,sizeof(tmpbuf)) != sizeof(tmpbuf)) {
#ifdef	ERRDEBUG
...
/* Determine if the new filename will fit.  If not, return 0. */
	if (_strlen(newfile) > MAXFILE) return (0);

/* Otherwise copy in the new file name. */
	else _strcpy(&utmpfile[0],newfile);
