...
#undef NSHIFT
int NSHIFT = 8;		/* log2(NINDIR) */
#undef NMASK
int NMASK = 0377;	/* NINDIR-1 */
long cur_ino = 2048;
...
/*  printf("version 1.4 - 12/15/80 %lu%\n\n");*/
	...
	if(argc!=2 && argc!=3) {
		printf("usage: fsdb /dev/dsk/c1d?s?\n");
		exit(1);
	}
...
#define S	sblock.fs
	...
	switch (sblock.fs.s_type) {
		case Fs1b:
			printf("%s(%.6s): 512 byte Block File System\n", argv[1], S.s_fname);
			BSIZE = 512;
			BSHIFT = 9;
			NSHIFT = 7;
			NMASK = 0177;
			...
			break;
		case Fs2b:
			printf("%s(%.6s): 1K byte Block File System\n", argv[1], S.s_fname);
			BSIZE = 1024;
			BSHIFT = 10;
			NSHIFT = 8;
			NMASK = 0377;
...
#endif /* end FsTYPE==2 */
#endif /* end !u3b2 && !u3b15 && !ns32000 && !clipper && !m88k */
