static char SysVNFSID[] = "@(#)tar.c	2.1 System V NFS source";
...
#if 1 
/* defines for POSIX tar --- POSIX manual says these should be in 
 * in the file <tar.h> --- do that later
*/
...
noupdate:
		...
		case 'h':
			if (speed == 1) {
				...
				break;	/* ignore symbolic link requests */
...
#ifdef TMAGIC
	(void) strcpy(dblock.dbuf.magic, TMAGIC);
