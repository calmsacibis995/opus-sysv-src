#ident "@(#) $Header: buf.c 4.1 88/04/26 16:00:39 root Exp $ RISC Source Base"
...
getbufhdr()
{
	...
	if(!Buf)
		if(!(Buf = symsrch("bufz")))
			error("bufz not found in symbol table\n");
...
char *heading;
{
	...
	long tmpbuf;	/* hold the address of the bufhdrs */

	/* n_value now references a pointer to bufheaders rather than the
	structures themselves.  Do one more level of indirection.   - sbw */
	if(lseek(mem, Buf->n_value, 0) == -1)
		error("seek error on address %x\n",Buf->n_value);
	if(read(mem, &tmpbuf, sizeof tmpbuf) == -1)
		error("read error on address %x\n",tmpbuf);
	fprintf(fp,"val=%x,  tmpbuf=%x\n", Buf->n_value, tmpbuf);
	/* end of new stuff */
	readbuf(addr,(long)(tmpbuf+slot*sizeof bhbuf),phys,-1,
...
getbuffer()
{
	...
	if(!Buf)
		if(!(Buf = symsrch("bufferz")))
			error("bufferz not found in symbol table\n");
...
long addr;
{
	long tmpbuf;	/* hold the address of the buffers */

	/* n_value now references a pointer to buffers rather than the
	structures themselves.  Do one more level of indirection.   - sbw */
	if(lseek(mem, Buf->n_value, 0) == -1)
		error("seek error on address %x\n",Buf->n_value);
	if(read(mem, &tmpbuf, sizeof tmpbuf) == -1)
	...
	fprintf(fp,"val=%x,  tmpbuf=%x\n", Buf->n_value, tmpbuf);
	/* end of new stuff */
	readbuf(addr,(long)(tmpbuf+slot*sizeof bbuf),phys,-1,
