...
#if m88k
#define	x_tvndx		x_fcnary.x_fcn.x_lnnoptr
#define x_tvfill	x_scnlen
#define x_tvlen		x_nreloc
#define x_tv		x_scn
#endif
	...
	for (	; first <= last;  ++first)
	{
		...
		if (secthead.s_nreloc != 0)
		{
			...
			for (i = secthead.s_nreloc; i > 0; --i)
			{
				...
				{
#if m88k
					printf("\t0x%.8lx  %10ld  %6d [0x%04x]\n",relent.r_vaddr,
						relent.r_symndx, relent.r_type, relent.r_offset);
#else
...
#if m88k
					else if (relent.r_type >= 128 && relent.r_type <= 133)
						printf("%s [0x%04x]", rtype88k[relent.r_type-128], relent.r_offset);
#endif
					else
						printf("UNKNOWN");
