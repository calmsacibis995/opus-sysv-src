/*
 * static char ID_symsh[] = "inc/common/syms.h: 1.5 2/24/83";
 */
...
#ifdef ns32000
	char		n_env;          /* symbol interpretation environment */
...
union auxent {
		...
		union {
			...
			struct {	/* if ISARY, up to 4 dimen. */
                ...
                union
                {
                       struct
                       {
			unsigned short  x_callseq;/* calling sequence info */
			unsigned short  x_level;  /* function nested level    */
                       }  x_fend;
                       struct
                       {
	...
	struct {
		...
		long            x_foff;    /* string stable offset of file name */
	} 	x_file;
        struct {
                long            x_scnlen;  /* section length  		   */
                unsigned short  x_nreloc;  /* number of relocation entries */
                unsigned short  x_nlinno;  /* number of line numbers       */
		long            x_linoptr; /* pointer to lin number entries
					      for this section             */
        }       x_scn;
	struct {
...
#if ns32000
#define	SYMESZ	sizeof(SYMENT)	           /* sizeof(SYMENT) */
...
#if m88k
#define	SYMESZ	sizeof(SYMENT)	           /* sizeof(SYMENT) */
#define	AUXESZ	sizeof(AUXENT)	           /* sizeof(AUXENT) */
#endif
