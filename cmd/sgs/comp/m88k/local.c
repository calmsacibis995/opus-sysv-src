	...
	*/
	...
	case SCONV:
		...
		if( p->in.left->in.op == ICON && !ISPTR(ml) ){
			...
			/* IEEE doesn't allow flt/double conversions
			   like the vax therefore .. */
			else if ( p->in.left->in.op == FCON && m == FLOAT )
				goto inherit;
			else if ( m==DOUBLE && ml==FLOAT ) break;
			else if ( m==FLOAT && ml==DOUBLE ) break;
			}
	...
	case LS:
	case ASG LS:
		/* force shift count to a range of 31-0 */

		p->in.right = buildtree( AND, p->in.right, bcon(31) );
		break;
...
incode( p, sz ) register NODE *p; {
	...
	word |= ((unsigned)(p->tn.lval<<(32-sz))) >> (inwd);
	inwd += sz;
	inoff += sz;
	if(inoff%SZINT == 0) {
		printf( "	word	0x%x\n", word);
 ...
 */
#define FAKENM 299	/* maximum number of fakenames */
...
#ifdef FLEXNAMES
	fprintf( outfile, "	sdef	%s\n", exname( p->sname ) );
#else
	fprintf( outfile, "	sdef	%.8s\n", exname( p->sname ) );
#endif
