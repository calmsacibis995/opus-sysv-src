...
# endif
	...
	case REGISTER:
		...
		if ( type == DOUBLE || type == FLOAT ) {
			fregvar -= 1; 
			p->offset = fregvar + 1;
			if( fregvar < minfrvar ) minfrvar = fregvar;
		}
		else {
			regvar -= cisreg(type);
			p->offset = regvar + 1;
		...
		if ( ISPTR(type) ) {
			pregvar -= 1; 
			p->offset = pregvar + 1;
			if( pregvar < minprvar ) minprvar = pregvar;
		}
		else {
			regvar -= cisreg(type);
			p->offset = regvar + 1;
...
#if CLIPPER
	minfrvar = fregvar = MAXFRVAR;
#endif
#if M68
	minprvar = pregvar = MAXPRVAR;
#endif
	reached = 1;
	swx = 0;
	swp = swtab;
...
#if CLIPPER
		else if ((cir = cisreg(type)) && ( (type == DOUBLE || type == FLOAT) ?
			fregvar >= MINFRVAR :  regvar >= MINRVAR - 1 + cir))
#endif
#if M68
		else if ((cir = cisreg(type)) && ( (ISPTR(type)) ?
			pregvar >= MINPRVAR :  regvar >= MINRVAR - 1 + cir))
#endif
#if !M68 && !CLIPPER
