		...
		if( *(cp=argv[c]) == '-' ){
			while( *++cp ){
				...
				case 'x':  /* general machine-dependent debugging flag */
					...
					break;
#if CLIPPER
				case 'A': /* check address alignment */
				case 'L': /* check load address alignment */
				case 'S': /* check store address alignment */
				{
					extern	laddrflt, saddrflt;
					extern safedbl;
					register c;
					c = *cp++;
					if (safedbl)
					cerror("illegal option combination");
					if ((*cp > '0') && (*cp < '8')) {
						if ( c == 'L' )
							laddrflt = *cp - '0';
						else if ( c == 'S' )
						...
						cerror ("bad option: %c%c",c,*cp);
				}
				case 'V': /* check addressing of arguments */
				{
					extern varargflag;
					varargflag = 1;
					break;
...
#endif
#ifdef MC88000
				case 'P': /* generate tb1 on function entry */
				{
					extern cpuflags;
					if (cp[1] == '\0')
						cpuflags = 1;
...
#endif
#ifndef FORT
				case 'F': /* allow floating arithmetic */
				{
					extern singflag;
					singflag = 1;
					break;
...
order(p,cook) NODE *p; {
	...
	if ( p->in.type == DOUBLE || p->in.type == FLOAT ) 
		MYREADER(p);	/* check for hardops again */
#endif
	cookie = cook;
	rcount();
	canon(p);
...
#if CLIPPER
	else if ((p->in.type == DOUBLE || p->in.type == FLOAT) &&
			(p->in.su > ffregs))
			mkadrs(p);
#endif
...
cbranch( p, true, false ) NODE *p; {
	...
	case GT:
		...
		&& ((p->in.left->in.op != REG)||(p->in.left->tn.rval <= D7))){
#else
		if( p->in.right->in.op == ICON && p->in.right->tn.lval == 0 && p->in.right->in.name[0] == '\0' ){
#endif
			switch( o ){
