...
#if M68
	if ( p->in.type == FLOAT ) p = makety(p, DOUBLE, 0, DOUBLE );
#endif
	return( p );
	}
...
oconvert(p) register NODE *p; {
	...
	case GT:
		...
		if( ISPTR(p->in.left->in.type) || ISPTR(p->in.right->in.type) )  p->in.op += (ULE-LE);
	case EQ:
	case NE:
		return( p );
...
tymatch(p)  register NODE *p; {
	...
	/* rules are:
		...
		if any float or doubles, make double if singflag == 0
			...
			if a float and an integer constant, make double to
			preserve precision
	...
	*/
	...
	if ((!singflag) &&
		 ( t1==DOUBLE || t1==FLOAT || t2==DOUBLE || t2==FLOAT ) ) 
			t = DOUBLE;
	else if ( t1==DOUBLE || t2==DOUBLE ) t = DOUBLE;
	else if ((t1==FLOAT && p->in.right->tn.op==ICON) ||
			(t2==FLOAT && p->in.left->tn.op==ICON)) t = DOUBLE;
...
prtdcon( p ) register NODE *p; {
	...
	if( p->in.op == FCON ){
		if( !SETDCON(p) ){ /* does code generator support FCON */
			...
			defalign( ALDOUBLE );
			deflab( i = getlab() );
#if N16 || CLIPPER || MC88000
			fincode( p->fpn.dval, p->in.type == DOUBLE ?
					SZDOUBLE : SZFLOAT );
#else
