...
# else
#	define symsiz 2000
/* Changed for 3.1 compatibility (11/22/88) */
/* #	define LINEFORM	"#line %d \"%s\"\n" */
#	define LINEFORM	"# %d \"%s\"\n"
#	define ERRFORM	"*%c*   \"%s\", line "
# endif
...
#    if defined(SGSINC) || defined(DMERT)
		if ( strlen( SGSINC ) != 0 )
			dirs[nd++] = relocate(SGSINC);	/* For cross cpp's */
#    endif
#    ifdef USRINC
		if ( strlen( USRINC ) != 0 )
			dirs[nd++] = relocate(USRINC);	/* usually /usr/include */
#    endif
#    if !defined( SGSINC ) && !defined( USRINC )
		dirs[nd++] = relocate("/usr/include");
#    endif
# endif
# if gcos
	dirs[nd++] = "cc/include";
...
/*
 *	The following allows cpp to be portable to both System V and
 *	BSD flavors of unix, without special makefile hacking...
*
...
/*
 *  Given pointer to a pathname, relocate to include sgsroot if
 *  necessary.
 */

static char *relocate (path)
char *path;
{
    auto char buff[512];

    if (*path == '/') {
	sprintf (buff, "%s%s", sgsroot, path);
    } else {
	sprintf (buff, "%s/%s", sgsroot, path);
    }
    return (copy (buff));
