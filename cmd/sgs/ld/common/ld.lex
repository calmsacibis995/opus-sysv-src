...
#if CLIPPER
#include "version.h"		/* ld version info for CLIPPER */
extern char CLI_version[];
extern char CLI_date[];
extern char CLI_cpu[];
#endif
 ...
 */
	...
	extern char *getldprefx();

	initpass1();
	ldprefx = getldprefx(argv[0]);
	argptr = argv;
	argcnt = argc;
	if ( argcnt <= 1 )
	{
...
#else
		fprintf( stderr, "%sld : %s : Version %s : date %s",
			SGS, RELEASE, CLI_version, CLI_date );
#endif
		if( Vflag == 2 ) {
#if AR16WR
...
#endif
#if CLIPPER
			fprintf( stderr, " : CLIPPER : ",CLI_cpu );
#endif
#if NS32000
			fprintf( stderr, " : NATIONAL-NS32000 : " );
#endif
#if M88K
			fprintf( stderr, " : MOTOROLA-M88000 : " );
#endif
#if AR32WR
