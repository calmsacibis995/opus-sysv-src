...
#else
			/* CLIPPER/M88K sections must begin on an eight-byte
			   boundary */
			if(ap->adscnptr->oshdr.s_paddr & 0x07L)
				lderror(0,0,NULL,"Section %.8s does not start on an eight-byte boundary!",ap->adscnptr->oshdr.s_name);
#endif
			}
...
/* CCF -- THIS SHOULD BE TRUE FOR THE M88K -- I THINK THE ASSEMBLER */
/* MAY BE GENERATING SOMETHING BOGUS -- SEE STATIC CODE BELOW */
...
#if CLIPPER
	aout.tcache = tcache;	/* text region caching policy */
	aout.dcache = dcache;	/* data region caching policy */
	aout.scache = scache;	/* stack region caching policy */
	aout.cliflags = 0L;	/* CLIPPER aouthdr flags unused at this time */
#endif
