...
#if CLIPPER
#include "aouthdr.h"
short tcache = 0;	 /* cache policy for text region	*/ 
short dcache = 0;	 /* cache policy for data region 	*/
short scache = 0;	 /* cache policy for stack region 	*/
short c_val;		 /* temporary used in assignments 	*/
#endif
 ...
 *
 *	-Ct|-Cd|-Csqq	: ****** CLIPPER only ******	
 *			  Ct : text caching policy
 *			  Cd : data caching policy
 *			  Cs : stack caching policy
 *			where qq :
 *				"pw"    private, write-thru (CLICPWT)
 *			        "sw"    shared, write-thru  (CLICCWT)
 *			  	"cb"	private, copy-back  (CLICPCB)
 *				"nc"    non-cacheable  	    (CLICNCA)
 *				"df"    default	            (CLICDFL)	
 *
 *	-F		: ****** UNIX only ******
...
#if CLIPPER
		case 'C':	/* Caching policies for CLIPPER regions */
			if (strncmp(&flgname[2],"pw",2)==0) c_val = CLICPWT;
			else 
			  if (strncmp(&flgname[2],"sw",2)==0) c_val = CLICCWT;
			  else 
			    if (strncmp(&flgname[2],"cb",2)==0) c_val = CLICPCB;
			    else 
			      if (strncmp(&flgname[2],"nc",2)==0) c_val = CLICNCA;
			      else
				if (strncmp(&flgname[2],"df",2)==0) c_val = CLICDFL;
				else
				  yyerror("Illegal option: %s  Options are pw, sw, cb, nc or df",&flgname[2]);
			...
			switch (flgname[1]) {
				...
				case 's':
					...
					break;
				default:
					yyerror("Illegal option: %c   Must specify Ct, Cd or Cs",flgname[1]);
			}
			break;
#endif
 ...
 */
	...
	{
		sprintf(libname, "%s%s%s%s%.8s%s", libdirs[dndx], "/",ldprefx,"lib", p, ".a");
...
char *cp;
{
  extern char *strrchr();
  int  cmdlen, preflen;
  static char prefix[30];          /*  30  should be enough  */
  char *prefptr, *ldname;
  ...
  preflen = cmdlen - strlen(ldname);

  strncpy(prefix, prefptr, preflen);  /*  assume preflen >=0 */
  prefix[preflen] = '\0';
  return (prefix);
}
