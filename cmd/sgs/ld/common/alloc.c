 ...
 */
 ...
 /*  NOTE:  The word align is a reserved word in the CLIPPER Green Hills
	    C compiler.  Thus, align in the following code was changed
	    to GH_align to indicate this fact.
 */
	...
	for( aip = (ACTITEM *) ownlist.head; aip != NULL; aip = nextaip ) {
		...
		if (aip->dfownr.aioutsec->osalign)
		{
			...
			cur_dot = -1L;
			GH_align = eval( aip->dfownr.aioutsec->osalign );
			chkpower2( GH_align );
 ...
 */
	...
	for( aip = (ACTITEM *) atownlst.head; aip != NULL; aip = nextaip ) {
		...
		if (aip->ownatr.aioutsec->osalign)
		{
			...
			cur_dot = -1L;
			GH_align = eval( aip->ownatr.aioutsec->osalign );
			chkpower2( GH_align );
		...
		for( mp = (MEMTYPE *) memlist.head; mp != NULL; mp = mp->mtnext ) {
			if( (aip->ownatr.aiownatt == mp->mtattr) &&
			    (can_alloc(mp->mtaddrhd,mp->mtaddrtl,
				     aip->ownatr.aioutsec,&ap1,&ap2,GH_align)) ) {
 ...
 */
	...
	for( usi = 0; usl[usi]; usi++ ) {
		...
		for( ani = 0; anl[ani]; ani++ )
			if( anl[ani]->adsize >=
			    (alignment( GH_align, anl[ani]->adpaddr)
			    + usp->oshdr.s_size) )
				break;
		...
		ap3 = anl[ani]->adnext;
		if( ! can_alloc(anl[ani], anl[ani], usp, &ap1, &ap2, GH_align)) {
...
/*eject*/
can_alloc(sap,eap,scp,pap1,pap2,GH_align)
ANODE *sap, *eap;
OUTSECT *scp;
ANODE **pap1, **pap2;
ADDRESS GH_align;
 ...
 */
	...
	while(!success) {
		...
		disp=alignment( GH_align, ap1->adpaddr );
...
long
alignment(GH_align,paddr)
ADDRESS GH_align ;	/* this will be a power or two */
ADDRESS paddr;
{
	register ADDRESS l;
