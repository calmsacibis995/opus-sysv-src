...
long buffer_size;
{
	...
	for ( reloc_read = 1; reloc_read <= isp->ishdr.s_nreloc; reloc_read++ )
	{
		...
		{
			lderror(1, 0, NULL, "relocation entry found for non-relocatable symbol in %s: %d",
				infl->flname,rentry.r_symndx);
			continue;
		}
...
/*    add new r_type for m88k    cplee      */
 ...
 /*   add new r_type for m88k    cplee */
                ...
                case R_LVRT16:
#if clipper || ns32000
                        value.c[0] = rentry.r_offset >> 8;
                        value.c[1] = rentry.r_offset & 0XFF;
#else
                        value.l = rentry.r_offset;
                        value.l <<=16;
#endif
                        value.c[2] = sect_buf[indx]; 
                        value.c[3] = sect_buf[indx+1]; 
                        break;
		...
		/* should check for overflow? mwc 2/12 */
 ...
 /*   add new r_type for m88k    cplee */
		...
		case R_DIR32S:
                case R_PCR16L:
                        if ( !rflag) {
                        tvalue = rdif - vaddiff - rentry.r_vaddr; 
                        tvalue1 = swapb4(value.l) & 0x0000FFFF;
                        tvalue += tvalue1; 
                        tvalue >>=2; 
                          tvalue += 1; /* temporary */
                        value.l &= swapb4(0xFFFF0000);
			value.l = swapb4(swapb4(value.l,0)+ tvalue, 0);
                        }
			break;
                case R_PCR26L:
		/*	value.l = swapb4(swapb4(value.l,0)+rdif-vaddiff,0);*/
                        if (!rflag) {
                        tvalue = rdif - vaddiff - rentry.r_vaddr; 
                        tvalue >>=2;
                        tvalue &= 0x03FFFFFF;
 ...
 /*   add new r_type for m88k    cplee */
                case R_VRT16:
                        value.l = value.l & swapb4(0x0000FFFF);
			sect_buf[indx] = value.c[2];
			sect_buf[indx + 1] = value.c[3];
                        break;
