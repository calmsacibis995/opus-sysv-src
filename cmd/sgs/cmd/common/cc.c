...
#endif
#if defined(M32) || defined(M32MAU) || defined(CLIPPER) || defined(NS32000) || defined(M88K)
#define CRT1	"crt1.o"
...
#ifdef CLIPPER
#define	OPTSTR	"Z:cpq:fOSW:t:EPgo:D:I:U:CHd:VQl:u:L:FK:Y:#"
#endif

#ifdef NS32000
#define	OPTSTR	"Z:cpq:fOSW:t:EPgo:D:I:U:CHVl:u:L:Y:#"
#endif

#ifdef M88K
#if DIAB
#define	OPTSTR	"Z:cpq:fOSW:t:EPgo:D:I:U:CHd:VQl:u:L:FK:Y:X:#"
#else
#define	OPTSTR	"Z:cpq:fOSW:t:EPgo:D:I:U:CHd:VQl:u:L:FK:Y:#"
#endif
#endif
...
#ifdef VAX
#define	OPTSTR	"cpq:fOSW:t:EPgo:D:I:U:CHVl:u:L:Y:#"
#endif

#ifdef MC68
#define	OPTSTR	"cpq:fOSW:t:EPgo:D:I:U:CHd:Vl:u:L:Y:#"
#endif

#ifdef M32
#define	OPTSTR	"cpq:fOSW:t:EPgo:D:I:U:CHd:VQl:u:L:FK:Y:#"
#endif
...
#if !defined(VAX) && !defined(CLIPPER) && !defined(NS32000) && !defined(M88K)
...
#else
#if defined(M32) || defined(M32MAU) || defined(CLIPPER) || defined(NS32000)
char	*crt	= CRT1;
#else
...
#if DIAB
		case 'X':	/* Optimization switch for DIAB compiler */
	...
	int i;
{
	...
	if (Pflag || Eflag)
		return(0);
#endif
#if defined(NS32000) || defined(CLIPPER) || defined(M88K)
	if (front_ret >= 1) {
#else
...
#if !defined(M88K) || defined(DIAB) /* m88k pcc doesn't have optimizer yet */
...
link ()
{
...
#if defined(CLIPPER)
	str= stralloc(strlen(LLIBDIR1) + 2);
	sprintf(str,"-L%s",LLIBDIR1);
	addopt(AV,str);			/* pcc uses /usr/lib for clipper */
#endif
	...
	if (gflag)
		addopt(AV,"-lg");
#endif
#if defined(M32) || defined(CLIPPER) || defined(NS32000)
	if (!Zflag)
...
#else
#if (defined(CLIPPER) || defined(M88K)) && !defined(DIAB)
		passc0 = makename( LLIBDIR1, prefix, N_C0 );
#else
...
#endif
#if defined(CLIPPER) || defined(M88K)
		passc2 = makename( LLIBDIR1, prefix, N_OPTIM );
#else
...
#else
#if defined(M32) || defined(NS32000)
		libploc= stralloc(strlen(LLIBDIR1) + 7);
		sprintf(libploc,"-L%s/libp",LLIBDIR1);
#else
#if defined(CLIPPER) || defined(M88K)
		libploc= stralloc(strlen(LLIBDIR1) + 9);
		sprintf(libploc,"-L%s/libpcc",LLIBDIR1);
#else
