...
mklibdef()
{
	...
	for (i=0; i<fhead.f_nscns; i++) {
		...
		/* MOTOROLA mwc 9/8/86 */
		/* added the %d and i parameter to the following sprintf */
		/* to avoid a name collision between the data and bss secs. */
		/* NOTE:  the following line contains a bug:  if the string */
		/* written is 8 characters, the null after it will overwrite */
		/* the next structure item, which may be s_vaddr */
		(void)sprintf(shead.s_name,".f%d%x",i, saddr);
...
archive()
{
	...
	p= cmd;
	p+= sprintf(p,"dir=`pwd` && cd %s && m88kar r $dir/%s `%slorder ",
