...
static int nrot = -1;		/* WARNING hangs the optimizer ( eg. ct.c ).
				   Happens with the vax compiler also.
			    	   Rotated loops */
...
#if NS32000
			if ( fortsrc )
			/* could be a fortran computed goto */
				p->op = HLABEL;
			else {
				DELNODE(p);
			}
...
/*	n += sizeof(char *) - ((int) n % (int) sizeof(char *));*/
	...
	n &= ~3;
	Maxu += n;
	if ( n >= ( (NSPACE-1) * sizeof(char *) ))
		p = xalloc(n);
	else while ((Lasta += n) >= Lastx) {
