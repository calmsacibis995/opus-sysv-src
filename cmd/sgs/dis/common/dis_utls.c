...
#if M88K
extern char lastlabelname[];
extern int lastlabelval;
#endif
/*
 *	line_nums ()
 *
...
line_nums()
{
	...
	while (next_function != NULL)
	{
...
#if M88K
		if (Aflag && (loc == Aflag)) {
			while (loc > next_function->faddr)
				if (next_function->nextfunc && loc > next_function->nextfunc->faddr) {
					next_function = next_function->nextfunc;
					FSEEK( t_ptr, (loc - scnhdr.s_paddr) + scnhdr.s_scnptr, 0);
				} else
					break;
		
			if (Sflag) {
				printf("%s:", next_function->funcnm );
		...
		if (Sflag)
			...
			printf("%s()\n", next_function->funcnm );
#if M88K
		strcpy(lastlabelname,next_function->funcnm);
		lastlabelval = next_function->faddr;
#endif
...

{
	...
	(oflag > 0) ?
		...
		printf("%-37s%s\n",object,mneu);	/* to print hex */
#endif
#if VAX || M32 || N3B || CLIPPER || NS32000 || M88K
#ifdef OLIST
	if (nosource == TRUE) return;
	printf("%-46s%-8s%s\n", object, linebuf, mneu);
#else
#if M88K
	if (Sflag > 0) /* Set up for assembler input */
		printf("%s\n",mneu);
	else
#endif
	if (oflag > 0)
...
symfind:
	...
	for (; loc < dumpaddr; ) {
		...
		get2bytes(t_ptr);
#endif
#if M32 || VAX || CLIPPER || NS32000 || M88K
		get1byte(t_ptr);
#endif
		sprintf(mneu,"");
		printline();
