...
char lastlabelname[128];
int lastlabelval = 0x10000000;
/*
...
#if M88K
	if (Sflag) {
		offset = val - lastlabelval;
		if (!found && (offset > 0) && (offset < 0x4000)) {	
			*pos+ = sprintf(*pos,"%s+0x%x",lastlabelname,offset);
			return;
		}
	}
#endif
...
char **pos;
{
	...
	if ((ualist != NULL) && (ptrua= ualist->next) != NULL) {
		do {
			tval= ptrua->symval;
...
#if CLIPPER || NS32000 || M88K
					*pos+=sprintf(*pos,(offset)?"(_%s+%ld)":"%s",
#else
...
#if CLIPPER || NS32000 || M88K
					*pos+=sprintf(*pos,"(_%s+%ld)",ptrua->name,
#else
...
char **pos;	/* postion in output string */
{
	...
	if ((regno == FPNO) || (regno == APNO)) {
		...
		*pos+=sprintf(*pos,"(");
		extsympr(val,pos);
		*pos+=sprintf(*pos,"+%s)",rname[regno]);
		found=1;
	}
	if (!found)
		*pos+=sprintf(*pos,"(%ld+%s)",val,rname[regno]);
}
