...
char **argv;
{
	...
	while( (ans = getopt( argc, argv, "aBC:ifopcdklmrSsbtuvVM:6eI:F:O:ZzL")) != EOF ) {
		...
		case 'C':		/* reset buffer size to arbitrary value */
			...
			if( Bufsize == 0 )
				errmsg( EERROR,
					...
					ans, optarg );
			/* round to integral number of 512 byte blks */
			Bufsize += 0x1ff;
			Bufsize &= ~0x1ff;
			break;
		case 'i':
		...
		case 'F':
			if(Option == OUT)
				goto Out;
			if(Option != IN) {
				fperr("F option needs either i or o flag\n");
				exit(2);
			}
			/* Fall Thru */
		case 'I':
	...
	if (zflag && Zflag)
		errmsg( EERROR, "-z and -Z cannot be used together.");
	switch(Option) {
		case IN:
			if (zflag)
				errmsg( EERROR, "-z and -i cannot be used together.");
			Cp = Cbuf = (char *)zmalloc(EERROR, Bufsize + CHARS + PATHSIZE) + CHARS + PATHSIZE;
			break;
		case OUT:
			if (Zflag)
				errmsg( EERROR, "-Z and -o cannot be used together.");
			...
			break;
		case PASS:
			if ((zflag || Zflag) && PassLink)
				errmsg( EERROR, "-l and -[zZ] cannot be used together.");
			if(Rename)
	...
	case OUT:
		...
		while(getname()) {
			...
			if (zflag && !A_special && !A_directory) {
				char *Zcmd[4096];

				close(Ifile);
				sprintf(Zcmd, "compress -c < %s", Hdr.h_name);
				if((Zfile = popen(Zcmd, "r")) == NULL) {
					fperr("cpio: popen() failed, cannot compress\n");
					exit(2);
				}
				Ifile = fileno(Zfile);
			}
			if ( Cflag )
				...
				bwrite(&Hdr, HDRSIZE+Hdr.h_namesize);
			for(filesz=mklong(Hdr.h_filesize); filesz>0; filesz-= rct){
				ct = filesz>CPIOBSZ? CPIOBSZ: filesz;
				errno = 0;
				if((rct=read(Ifile, Buf, (unsigned)ct)) < 0) {
					/*
					** If the read failed, write a block of zeros
					** to keep the tape in sync with the file header.
					*/
					memset(Buf, '\0', ct); 
					bwrite(Buf,ct);
					fperr("Cannot read %s\n", Hdr.h_name);
	...
	case PASS:		/* move files around */
		...
		while(getname()) {
			...
			if (Zflag) {
				...
				close(Ifile);
				sprintf(Zcmd, "uncompress -c < %s", Hdr.h_name);
				if((Zfile = popen(Zcmd, "r")) == NULL) {
					fperr("cpio: popen() failed, cannot uncompress\n");
					exit(2);
				}
				Ifile = fileno(Zfile);
			}
			filesz = Statb.st_size;
			for(; filesz > 0; filesz -= rct) {
				ct = filesz>CPIOBSZ? CPIOBSZ: filesz;
				errno = 0;
				if((rct=read(Ifile, Buf, (unsigned)ct)) < 0) {
					fperr("Cannot read %s\n", Hdr.h_name);
...
ckverbose:
	...
	fperr("%ld blocks\n", Blocks * (Bufsize>>9));
	/* new - needs to be fixed: fperr("%ld blocks\n", ((Blocks/(Bufsize>>9) + 1) * (Bufsize>>9))   ); */
	exit(0);
}
...
{
	errusage("%s\n\t%s %s\n\t%s %s\n\t%s %s\n\t%s %s\n",
		    "-o[acvVBz] [-Cbufsize] [-Mmessage] <name-list >collection",
	Err.source, "-o[acvVBz] -Ocollection [-Cbufsize] [-Mmessage] <name-list",
	Err.source, "-i[bcdkmrsStuvVfB6Z] [-Cbufsize] [-Mmessage] [pattern ...] <collection",
	Err.source, "-i[bcdkmrsStuvVfB6Z] -Icollection [-Cbufsize] [-Mmessage] [pattern ...]",
	Err.source, "-p[adlmruvVzZ] directory <name-list");
}
...
getname()		/* get file name, get info for header */
{
	...
	for(;;) {
		...
		if ((Option == OUT && zflag) || (Option == PASS && Zflag)
		    && tlong && !A_directory && !A_special) {
			char *Zcmd[4096];
			int rct;

			if (Option == OUT)
				sprintf(Zcmd, "compress -c < %s", namep);
			else
				sprintf(Zcmd, "uncompress -c < %s", namep);
			if((Zfile = popen(Zcmd, "r")) == NULL) {
				fperr("cpio: popen() failed, cannot (un)compress %s\n", namep);
				exit(2);
			}
			tlong = 0;
			do {
				rct = read(fileno(Zfile), Zcmd, 4096);
				if (rct < 0) {
					pclose(Zfile);
					Zfile = NULL;
					fperr("cpio: cannot read %s\n", namep);
					exit(2);
				}
				tlong += rct;
			} while (rct);
...
/* try creating (only twice) */
	...
	if(Uid == 0)
		zchown( EWARN, namep, Hdr.h_uid, Hdr.h_gid);
	if ((Option == IN && Zflag) || (Option == PASS && zflag)) {
		char *Zcmd[4096];
		...
		if((Zfile = popen(Zcmd, "w")) == NULL) {
			fperr("cpio: popen() failed, cannot (un)compress\n");
			exit(2);
		}
		f = fileno(Zfile);
	}
...
again:
	...
	if((f = open(str, x?O_WRONLY|O_CREAT|O_TRUNC:O_RDONLY, 0666)) < 0) {
