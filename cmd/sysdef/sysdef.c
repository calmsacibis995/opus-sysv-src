...
#define MEMSEEK(nl)	lseek(memf, (long) SYM_VALUE(nl), 0)
#define MEMREAD(var)	read(memf, (char*)&var, sizeof(var))
#else
...
#if ns32000
#undef FBOMAGIC
#define FBOMAGIC	NS32WRMAGIC
#endif
#if clipper
#undef FBOMAGIC
#define FBOMAGIC	CLIPPERMAGIC
#endif
#if m88k
#undef FBOMAGIC
#define FBOMAGIC	MC88MAGIC
#endif
...
#if clipper || ns32000 || m88k
	nlptr->n_name = malloc((unsigned)(strlen(nam)+3));	/* initialize pointer to next string */
	if (*nam) {
		*(nlptr->n_name) = '_';
		strcpy(&(nlptr->n_name[1]),nam);/* move name into string table */
	}
	else {
		nlptr->n_name = nam;
	}
