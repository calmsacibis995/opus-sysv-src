...
char	*argv[];
{
	...
	while ((c = (int)getopt(argc, argv, "xufnbd:m:v:s:o:c:")) != EOF)
		...
		default:		/* illegal option letter */
			fprintf(stderr,"Usage: %s [-m name] [-uxb]\n",cmdname);
			exit(1);
		}
 ...
 */
	...
	if((strcmp("/etc/master.d/kernel",master_name) == 0))
		exit_msg("ERROR:Cannot install/uninstall \"kernel\"");
 ...
 * If driver is being installed, adds a define
 * statement.
 * If driver is being uninstalled, adds an undefine
 * statement.
 *
 */
...
do_system()
	...
	while(fgets(masbuf, (int)MYBSIZE, systemfile) != NULL) {
		strcpy(tmpbuf,masbuf);
		if((strncmp("#define", (char *)masbuf, 7) == 0) ||
		   (strncmp("#undef", (char *)masbuf, 6) == 0)) {
			if ((argc=parse(args,50,masbuf)) > 50)
				myerror(argc,argv);
			if (argc==1)
				myerror(argc,argv);
			argc -=1;
			if( 0 == (strcmp(capital_name, argv[1])) ) {
				continue;
			} else {
				strcpy(masbuf,tmpbuf);
			}
		}
