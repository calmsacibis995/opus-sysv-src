static char SysVNFSID[] = "@(#)acctdusg.c	2.1 System V NFS source";
...

{
	...
	while ((pwentry = (pfile ? fgetpwent(pswd) : getpwent())) != NULL) {
		makdlst(pwentry);	/* make a list of home directory names
...
register struct passwd *p;
{
	...
	strcpy(dp->dsk_name, p->pw_name);	/* copy login name from 
