...
#ident	"@(#)hdelogger.c	1.5	10/6/86 Copyright (c) 1986 by National Semiconductor Corp."
...
discard:
		...
		if (edpdck())
			goto discmsg;
#ifdef	HDELOG
		if (memcmp(newrep.dskserno, edpdp->dt_serial, PDNSIZE)) {
#else	/* HDELOG */
		if (memcmp(newrep.dskserno, edpdp->pdinfo.serial, 12)) {
#endif	/* HDELOG */
			printf(
			"%s: media serial number of disk maj=%d min=%d\n",
				cmdname, major(newrep.diskdev),
				minor(newrep.diskdev));
			printf(
			"\tdisagrees with serial number in error report\n");
			goto discmsg;
		}
#ifdef	HDELOG
		if (newrep.blkaddr >= edpdp->dt_defectst &&
			newrep.blkaddr < edpdp->dt_defectst +
			(edpdp->dt_defectsz + edsecsz - 1) / edsecsz)
#else	/* HDELOG */
...
#ifdef	HDELOG
		if (newrep.blkaddr >= edpdp->dt_errlogst &&
			newrep.blkaddr < edpdp->dt_errlogst +
			(edpdp->dt_errlogsz + edsecsz - 1) / edsecsz)
#else	/* HDELOG */
