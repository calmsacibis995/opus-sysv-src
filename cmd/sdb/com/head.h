...
extern INT	gflag;			/* signifies existance of debug syms */
...
#define	COMMANDS	"\004\"+-=!</?BCDIMQRSTUVXYabcdegiklmpqrstvwxz"
				/* each sdb command must appear here */
#define SUBCMDS		"\004\"+-=</?BCDIMQRSTUVXYabcdegiklmpqrstvwxz"
				/* all commands which cannot have ':' in arg */
#if (!clipper) && (!n32000) && (!m88k)
...
#if clipper || m88k
	ADDR	paddrbf;	/* core addr of .bf (ie 1st executable instr */
#endif /* clipper */
