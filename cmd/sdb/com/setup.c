...
setcor()
{
	...
	if(fcor < 0)
	{
	   ...
	   /*  sure a core file */
#if m88k
	if (read(fcor, uu, sizeof(struct ptrace_user))==sizeof(struct ptrace_user)) {
#else
...
#endif
#if m88k
		signo = ((struct sigframe *)(((struct user *)uu)->pt_sigframe))->sf_signum;
		txtsiz = ((struct user *)uu)->u_tsize;
		datsiz = ((struct user *)uu)->u_dsize;
		stksiz = ((struct user *)uu)->u_ssize;
#else
 ...
 */
...
#if clipper || ns32000
		datmap.f1 = scnhdrp[libn][dat].s_scnptr % NBPC + XTOB(USIZE);
#else
#if m88k
		datmap.f1 = (ADDR)((struct user *)uu)->pt_dataptr;
#else
		...
		datmap.b2 = (ADDR)USRSTACK - stksiz;
#else
		/*  on vax, stksiz includes sizeof user area - must add back */
		datmap.b2 = USRSTACK + XTOB(USIZE) - stksiz;
#endif
		datmap.e2 = (ADDR)USRSTACK;
#endif
#if m88k
		datmap.f2 = (ADDR)((struct user *)uu)->pt_dataptr + (magic==OMAGIC?datsiz+txtsiz:datsiz);
#else
