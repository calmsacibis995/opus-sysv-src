...
char *p;
{
	...
	if (percentflag) {
		...
		if (ISREGV(sl_class)) {	 /* sl_class only valid for variables */
			...
			if (ch == 'g') {
#if clipper || ns32000 || m88k
 				rhs_value.val.ww.w1 = getval(j, "d", DSP);
 				rhs_value.val.ww.w2 = getval(j+WORDSIZE, "d", DSP);
#else
 			...
 			{
 			    unsigned *fptr ;
 				fptr = (unsigned *)&rhs_value.val.f ;
 				*fptr = getval(j, "d", DSP) ;
 			}
 			else
 			{
 				rhs_value.val.i = getval(j, typeptr, DSP);
#else
			else {
