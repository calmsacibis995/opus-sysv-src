...
/* breakpoint instruction */
#define BPT	0x127f
#define BPTSIZE	0xffff
#define exect execve		/* exect is obsolete on the clipper */
#endif
#endif
...
setup()
{
	...
	if ((pid = fork()) == 0)
	{
		...
		/* read uu->u_ar0 */
#if clipper
		((struct user *)uu)->u_ar0 = (int *)ptrace(RUREGS, pid,
			(char *)&(((struct user *)uu)->u_ar0) - (char *)uu, 0);
		if (((int)(((struct user *)uu)->u_ar0) == -1) && errno)
#else
#if m88k
		if (OPUSVENDORID != ptrace(RUREGS, pid,
			(char *)&(((struct user *)uu)->pt_vendor) - (char *)uu,
					0))
#else
...
BKPTR	bkptr;
{
	...
	struct proct *procp;
		...
		else if (debugflag == 2)
		{
			...
			arg("bkptr");
			printf("0x%x",bkptr);
			printf(", [bkptr->loc = 0x%x]",bkptr->loc);
			closeparen();
		}
#endif
	adr = bkptr->loc;
...
BKPTR	bkptr;
{
	...
	if (!(errno && ptv == -1)) {
		bkptr->ins = ptv;
#if clipper
		ptv = ptrace(WIUSER, pid, adr, BPT | ptv &~BPTSIZE);
#else
#if m88k
		ptv = ptrace(WIUSER, pid, adr, BPT);
#else
...
#endif
#if ns32000
	for (i=0; i < NUMFREGS; i++)
	{
		SDBFREG(i) = ptrace(RUREGS, pid, FPREG(i), 0);
	}
#endif
#endif
...
xptrace(a,b,c,d) {
	...
	printf("ptrace(%x,%x,%x,%x) ",a,b,c,d);
	retval = ptrace(a,b,c,d);
 	printf("returned %x\n",retval);
	return(retval);
}
