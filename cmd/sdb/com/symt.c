...
#if clipper
	badproc = procp;	/* back up to the first unused entry */
	badfile = filep;	/* back up to the first unused entry */
...
#if m88k
			    /* ignore any compiler local labels */
			    (syp->n_nptr[0] != '@' || syp->n_nptr[1] != 'L') &&
#endif
...
#if clipper
			procp->paddrbf = 0;	/* set if .bf encountered */
#endif
...
#if !defined(m88k)	/* otherwise, we can't look at dissambly easily */
...
ADDR addr; {
	...
	static ADDR nextprocaddr; /*  address of next proc so we can match */
				  /*  all addresses in this function */
	...
	/* this often gets called with addr - 1, and cacheing the value */
	/*  should help performance substantially */
	if (addr & 1) {
		struct proct * retval; 
		retval = adrtoprocp2(addr);
#if DEBUG
		if (debugflag == 1)
		{
			exit1();
	...
	if(addr == savelast.sadr || (addr > savelast.sadr && addr < nextprocaddr )) {
#if DEBUG
		if (debugflag == 1)
		{
			exit1();
...
#if m88k || clipper || ns32000
		if (a->notstab && b->notstab)
		{
			result = 0;
		}
		else if (a->notstab)
...
char *pnam; struct stmt stmt; {
	...
	if (*s == '\0')
	{
		...
		{
			exit2("getaddr");
			/*printf("0x%x",addr); addr not set yet!!! */
			endofline();
		}
#endif
		return(ERROR);
