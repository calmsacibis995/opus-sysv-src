...
int space;
{
	...
	if (dfmt == 's')  
	{
		...
		if (addr > 0x80000000) {
#endif
#if u3b5 || u3b15 || u3b || vax || clipper || ns32000 || m88k
	...
	if (prvar == 3) {	/* assign rhs_value to variable */
		...
		if (ISREGV(class)) {
			...
			case 4:		/* FLOAT, FLOAT */
                          ...
                          /* Have to copy it bit by bit, so the best way */
                          /* is to treat it as an integer.               */
				putreg(addr, 'i', rhs_value.val.i);
#else
...
printregs()
{
	...
	for (p=reglist; p < &reglist[(NUMREGLS+1)/2]; p++) {
		printf("%4.4s/  ", p->rname);
		prhex12(SDBREG(p->roffs));
		if (p + (NUMREGLS+1)/2 < &reglist[NUMREGLS]) {
			printf("\t%7.7s/  ",(p+(NUMREGLS+1)/2)->rname);
			prhex12(SDBREG((p+(NUMREGLS+1)/2)->roffs));
		}
		printf("\n");
	}
#if ns32000
	if (SDBFREG(0)) {
		int i;
		printf("%-27s%-27s\n","float","double");
		for (i=1; i < NUMFREGS; i++) {
			printf("  f%d/  ",i-1);
			if (NaF(*(float *)&SDBFREG(i)))
				printf("%-20s","????");
			else
				printf("%-20e",*(float *)&SDBFREG(i));
			if (i&1) {
				printf("  f%d/  ",i-1);
				if (NaD(*(double *)&SDBFREG(i)))
					printf("%-s\n","????");
				else
					printf("%-e\n",*(double *)&SDBFREG(i));
			} else
