...
#if m88k
			for (i=0; i<sizeof(struct ptrace_user); i+=4) {
#else
			for (i=0; i<USIZE*NBPP; i+=4) {
#endif
				if (i % 32 == 0)
					printf("\n%.4x: ",i);
				retval = ptrace(RUREGS, pid, i, 0);
				printf("%.8x ",retval);
			}
		}
		break;
...
/*  TEMPORARY ****	put back !! ***** */
...
#endif
#if u3b || vax || clipper || ns32000 || m88k
			SDBREG(FP) = fps;
#if u3b || vax
			SDBREG(AP) = aps;
#endif
	...
	case '?':
		...
		else
		{
			...
			if ((int)dot == -1) {
				if (proc[0])
					fprintf(FPRT1, "%s:", proc);
				if (var[0])
					fprintf(FPRT1, "%s", var);
				fprintf(FPRT1, " not found\n");
				break;
			}
#endif
		}
	...
	case '/':
		...
		else
		{
			...
			{
#if clipper || m88k
				if ((int)dispvar(proc, var, args) == -1) {
					if (proc[0])
						fprintf(FPRT1, "%s:", proc);
					if (var[0])
						fprintf(FPRT1, "%s", var);
...
#if u3b2 || u3b5 || u3b15 || ns32000 || clipper || m88k
#define SAVE	0x10
#define ADDW2	"ADDW2"
#endif
#if u3b
