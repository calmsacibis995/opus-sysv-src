...
#		UNIX System V Release 3 (nonpaging and paging)
...
sdb:
	if vax ; then cd vax ; 					\
		...
		$(MAKE) -f makefile				\
			...
			DEBUG="$(DEBUG)"			\
			TESTDIR="$(TESTDIR)" CC="$(CC)" ;	\
	elif ns32000 ; then cd ns32000 ; 			\
		$(MAKE) -f makefile				\
			FLEX="$(FLEX)"				\
			ROOT=$(ROOT) 				\
			CFLAGS="$(FFLAG) $(CFLAGS) -DMMU_TYPE=32082" \
			DEBUG="$(DEBUG)"			\
			TESTDIR="$(TESTDIR)" CC="$(CC)" ;	\
	elif m88k ; then cd m88k ; 			\
		$(MAKE) -f makefile				\
			FLEX="$(FLEX)"				\
			ROOT=$(ROOT) 				\
			CFLAGS="$(FFLAG) $(CFLAGS)" \
