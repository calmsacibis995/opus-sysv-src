static char SysVNFSID[] = "@(#)ls.c	2.1 System V NFS source";
...
struct lbuf *ap;
{
	...
	if (lflg) {
		...
		if (oflg)
			if(!nflg	/* Cache the last user name. */
			   && ((pwentry != NULL
				&& pwentry->pw_uid == p->luid)
			       || (pwentry = getpwuid(p->luid)) != NULL
			       )
			   )
				curcol += printf("%-9.9s", pwentry->pw_name);
			else
				curcol += printf("%-9u", p->luid);
		if (gflg)
			if(!nflg	/* Cache the last group name. */
			   && ((grentry != NULL
				&& grentry->gr_gid == p->lgid)
			       || (grentry = getgrgid(p->lgid)) != NULL
			       )
			   )
				curcol += printf("%-9.9s", grentry->gr_name);
			else
