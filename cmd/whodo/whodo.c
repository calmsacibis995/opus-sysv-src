...
#include   <sys/region.h>
#include   <sys/sbd.h>
#endif /* u3b2, ns32000, clipper || m88k */
...
#if pdp11 || vax || ns32000 || clipper || m88k
...
register struct proc *p;
{
	...
	ww = ptosv(p->p_addr) + UDOT_P*sizeof(struct pde);
	lseek(fd,(long)ww,0);
	read(fd,udotpde, sizeof(udotpde));
	for(c=0; c<USIZE; c++) {
		lseek(fd, udotpde[c].pgi.pg_pde & (~POFFMASK), 0);
		if (read(fd,(char *)(((char *)&uarea)+NBPP*c),NBPP) != NBPP)
					/* get upage  */	
			return(0);
...
#if m88k
        addr = ctob(btoct(p->p_ubpgtbl[0].pgi.pg_spde));
        lseek(fd, addr, 0);
        if (read(fd, (char *)&uarea, sizeof(uarea)) != sizeof(uarea))
                return(NULL);
#endif
