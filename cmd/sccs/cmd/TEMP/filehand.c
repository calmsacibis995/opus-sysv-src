...
begin:
	...
	if ((opcode & INSERT) > 0) {
		...
		retcode = ABEND;				 %d: replacement out of bounds  -----rule matches:  ----- rule fails
   
   , rp=   ap= -----trying rule:   rewrite: ruleset %2d   input:   ()<>,;\"
 o  Unbalanced '"'  Unbalanced '<'  Unbalanced '('  prescan: too many tokens    tok=    ns=%02o
    Unbalanced '>'  Unbalanced ')'  c=%c, s=%d;     Address too long    prescan:    parseaddr-->            cannot resolve name 
--parseaddr(%s)
               xfAA01338                                                                                                                                                                                                                                                         :    @(#)queue.c	5.24 (Berkeley) 6/30/88 (with queueing) %s: unlock  queuename: %s
  %cf%s   %s: assigned id queuename: assigned id %s, env=%x
  queuename: Cannot create "%s" in "%s"   queuename: trying "%s"
 qfAA%05d    
    (no control file)  
				  %.45s    
					 %.38s    
		 (%.60s) %8ld %.16s %.45s    %8ld %10ld %.12s %.38s      X   *   %7s r   )
--ALLOC3) > 0) {
		TR("Sweep: free arg\n", EMPTY, EMPTY, EMPTY);
		free ((char*) arg);
		}
