...
#ifdef SENDMAIL
char	rfrom[] = "From:";	/* from-line sentinel */
#endif
int	from_len = 0;
...
#ifdef SENDMAIL
	/*
		Is sendmail properly installed here.
	*/
	doingsm = (access(SENDMAIL, A_EXECUTE) != CERROR) &&
		(access("/usr/lib/sendmail.cf", A_READ) != CERROR);
#endif
	/*
...
#ifdef SENDMAIL
	/*
	 * An address cannot have these characters (even for a remote system)
	 */
	if (p = strpbrk(name, "`;&|\n"))
#else
	if (p = strpbrk(name, "`;&|^<>()\n"))
...
#ifdef SENDMAIL
	/*
	 * Ensure that the address is really local; else sendmail will
	 * take care of the rest
	 */
	if ((sp=strchr(saved_name, '!'))) {
		local = NULL;
		sp++;
	}
	else if ( strpbrk(saved_name, "@.`;&|^<>()\n") == NULL) 
		local = 1;
	else  
		local = NULL;
#endif
...
#ifdef SENDMAIL
		if (doingsm) {
			sprintf(cmd, "exec /usr/lib/sendmail %s  2> /dev/null", 
				sp?sp:saved_name);
			if (pipletr(n,cmd,local) == TRUE) {
				return(TRUE);
			/* if this fails UUCP will do the rest */
			}
		}
#endif
...
{
	int	i, w;
	char	buf[128], *cp, work[128], wuser[128], saved_line[128];
#ifdef SENDMAIL
	int done = 0;
	char *p;
#endif
	...
	if (dflag < 2) {
		...
		for (fgets(line,LSIZE,tmpf); 
			strncmp(line,from,5) == SAME || 
			  strncmp(line +1,from,5) == SAME;) {
#endif /* SENDMAIL */
#ifdef SENDMAIL
			for(p=line; *p == ' ' || *p == '\t' || *p == '>'; ++p)
				;
			...
			else if (strncmp(p, "Received:", 9 ) == 0) {
				goto next;
			}
			else if (strncmp(p, "Date", 4 ) == 0) {
				goto next;
			}
			else if (strncmp(p, "Messgae-Id", 10 ) == 0) {
				goto next;
			}
			else if (strncmp(p, "Apparently-To", 13 ) == 0) {
				goto next;
			}
			else if (strncmp(p, "Subject", 7 ) == 0) {
				goto next;
			}
