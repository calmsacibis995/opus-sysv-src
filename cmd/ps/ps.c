...
#ident	"@(#)ps.c	1.2	5/22/86 Copyright (c) 1986 by National Semiconductor Corp."
...
#include "sys/types.h"
#if u3b2 || u3b5 || clipper || ns32000 || m88k
...
#if u3b || u3b2 || u3b5 || clipper || ns32000 || m88k
struct proc prc;
#define	mproc	prc
#define	zproc	prc
#endif /*u3b,u3b2,u3b5,clipper,ns32000,m88k*/
...
#if pdp11
	getstr = "lfeadn:s:c:t:p:g:u:";				/* note s option */
	coref = "/dev/mem";
	memf = coref;
#endif
...
#if ns32000
		/* ww <= physical address of pde for process's ublock */
	ww = ptosv(mproc.p_addr) + UDOT_P*sizeof(struct pde);
	l_lseek(swmem,(long)ww,0);
	r_read(swmem,udotpde, sizeof(udotpde));
	for(c=0; c<USIZE; c++) {
		l_lseek(swmem, udotpde[c].pgi.pg_pde & (~POFFMASK), 0);
		if (read(swmem,(char *)(((char *)&u)+NBPP*c),NBPP) != NBPP)
					/* get upage  */	
			return(0);
		if(sizeof(struct user) - (c + 1) * NBPP <= 0)
			break;
	}
#endif
...
#if clipper
	for(c=0; c<USIZE; c++) {
		l_lseek(swmem, mproc.p_upte[c].pgi.pg_pde & (~POFFMASK), 0);
		if (read(swmem,(char *)(((int *)&u)+1024*c),NBPP) != NBPP)
			return(0);
		if(sizeof(struct user) - (c + 1) * NBPP <= 0)
			break;
	}
...
#if m88k
	for(c=0; c<USIZE; c++) {
		l_lseek(swmem, mproc.p_ubpgtbl[c].pgi.pg_spde & (~POFFMASK), 0);
		if (read(swmem,(char *)(((int *)&u)+1024*c),NBPP) != NBPP)
			return(0);
		if(sizeof(struct user) - (c + 1) * NBPP <= 0)
			break;
...
#if m88k
		printf("%9x%7d", mproc.p_pttbl, mproc.p_size);		/* ADDR SZ */
		if (mproc.p_wchan)
			printf("%9x",mproc.p_wchan);			/* WCHAN */
		else 
			printf("         ");
...
#if vax || u3b5 || clipper || ns32000
		printf("%9x%7d", mproc.p_addr, mproc.p_size);		/* ADDR SZ */
		if (mproc.p_wchan)
			printf("%9x",mproc.p_wchan);			/* WCHAN */
		else 
			printf("         ");
#endif /*vax,u3b5,clipper,ns32000,m88k*/
...
#if vax || u3b || u3b2 || clipper || ns32000 || m88k
...
#endif /*u3b,vax,u3b2,clipper,ns32000,m88k*/
