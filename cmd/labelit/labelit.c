...
/* labelit with 512, 1K, 2K, 4K, and 8K block size support */
#include <stdio.h>
#include <sys/param.h>
#ifndef	RT
#include <signal.h>
...
#define	S	super.f.fs
	if (S.s_magic == FsMAGIC) {
		...
		case Fs2b:
			...
			printf("Current fsname: %.6s, Current volname: %.6s, Blocks: %ld, Inodes: %d\nFS Units: 4Kb, ", 
			S.s_fname,S.s_fpack,S.s_fsize * 8,(S.s_isize - 2) * 64);
		    else if(result == Fs16b)
			printf("Current fsname: %.6s, Current volname: %.6s, Blocks: %ld, Inodes: %d\nFS Units: 8Kb, ", 
			S.s_fname,S.s_fpack,S.s_fsize * 16,(S.s_isize - 2) * 128);
		    else {
...
long	blksize;
{
	 ...
	 *   or 8192 (indicating 4KB block size)
	 *   or 16384 (indicating 4KB block size)
	 */
	for(count = 1; count < 9; count++) {
