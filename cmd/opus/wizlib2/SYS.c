...
register struct iob *io; {
	...
	l3tol((char *)io->i_addr, (char *)dp->di_addr, NADDR);
}
...
register struct iob *io; {
	...
	if (lseek(io->i_ino.i_dev, io->i_bn * 512, 0) < 0)
		return (-1);
	return(XREAD(io->i_ino.i_dev, io->i_ma, io->i_cc));
}
...
_devopen(io)
register struct iob *io; {
	static struct dtab dt;

	io->i_ino.i_dev = XOPEN("/dev/dsk/c1d0s0", 0);
	io->i_dp = &dt;
}

_devclose(io)
register struct iob *io; {
	XCLOSE(io->i_ino.i_dev);
}
