...
char **argv;
{
	...
	while ((c = getopt(argc,argv,"thd:")) != EOF)
		...
		case 'd':
			...
			if (errflg)
				break;
#if u3b || u3b5 || clipper || ns32000 || m88k
			temp = 1;
#else
#if u3b2
...
#if clipper || ns32000 || m88k
	/* buf is actually a pointer */
	lseek(f,(long)setup[SBF].n_value,0);
	if ( read (f, &i, 4) != 4 )
		extmsg("cannot read buffer pointer address");
	setup[SBF].n_value = i;
#endif
...
#if clipper || ns32000 || m88k
	if (dflg == 0){
		if (setup[IDTAB].n_value != 0 )
			dev = 0;
		else
			extmsg("id is not defined");
		for (i=0;i<NDRIVE;i++)
			drvlist[i] =1;
	}
	else 
...
#if pdp11 || vax || u3b2 || clipper || ns32000 || m88k
