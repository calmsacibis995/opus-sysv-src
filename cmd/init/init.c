...
struct	termio	dflt_termio = {
	...
	ISIG|ICANON|ECHO|ECHOK,
	0,
	control(C),control(\\),control(H),control(U),control(D),0,0,0
};
#else
struct	termio	dflt_termio = {
	BRKINT|IGNPAR|ISTRIP|IXON|IXANY|ICRNL,
...
#if m88k
			/* override, if unix boot specifies level */
			i = _sys_local( 0xf00, 17);
			/* only if /etc/secure is no present */
			if (i != -1 && access("/etc/secure", 0) == -1)
				initstate = states[i];
#endif
...
** Austin - Change made by PEG 1/18/88
**
**		if (*program == 'S')
**
*/
		if (program != NULL && *program == 'S' && oldu != NULL)
