...
char **argv;
{
int i, c, status, gflg = 0,cdbflag = 0;
...
#endif
#if clipper
		case 's':		/* vax alignment */
			vaxalign();
			p2options = "-C3";
			goto copyfflag;
#endif
#if TARGET == OPUS && m88k
		case 'P':		/* pass2 options */
			p2options = s - 1;
			s++;
			while (isdigit(*s)) s++;
			s--;
...
for(i = 0 ; i<argc ; ++i)
	...
	case 's':	/* Assembler file */
		...
		if (access(ccname, 1) == 0)
			sprintf(buff,"%s -c %s",ccname,argv[i]);
#ifdef CCNAME1
		else if (access(CCNAME1, 1) == 0)
			sprintf(buff,"%s -c %s",CCNAME1,argv[i]);
#endif
		else {
			fprintf(stderr, "%s: Cannot load C compiler\n", argv[0]);
			erred = YES;
			break;
		}
...
sprintf(buff,"%s %s <%s >>%s", pass2name, p2options, textfname, asmfname);
if (sys(buff))
	goto comperror;
...
#	endif
#if TARGET == OPUS
	if (content(pass2opt) == -1) {
		fprintf(stderr,"Can't exec %s\n",pass2opt);
		*lastc = 'f';
		fprintf(stderr,"Optimizer failed, -O ignored for %s\n",s);
		*lastc = 'o';
		} 
	else 
#endif
...
#if TARGET == OPUS && m88k
	sprintf(buff, "%s -u -o %s %s", asmname, obj, afile);
#else
...
c = *lastc;
*lastc = 'x';
sprintf(buff,"sort +0 -1 +1 -2 +2 -3 -n xref.out | uniq | /usr/lib/fxref > %s",
				xref);
system(buff);
rmf("xref.out");
*lastc = c;
