...
#if TARGET == OPUS
/* 
	DO 10 VAR = DOINIT, DOLIMIT, DOINCR
	In the above do statement if VAR appears in DOINIT, DOLIMIT or DOINCR
	the compiler generates bad code. This recursive Fix, looks for VAR 
	in an expression.
	If VAR is present in the exppression return TRUE else return FALSE.
	exdo will not put VAR into a register if findnp returns TRUE.
*/
...
switch (ep->tag)
	...
	case TNAME:
		return(np == (Namep) ep);
	case TEXPR:
		return (findnp(np, ep->exprblock.leftp) || 
				findnp(np, ep->exprblock.rightp));
	case TPRIM:
		p = (struct Primblock *)ep;
		if (p->argsp)
		...
		return(findnp(np, p->namep));
	default:
		fprintf(stderr, "findnp: unknown tag %d\n", ep->tag);
		return 1;
	}
}
#endif
