...
#if TARGET == VAX || TARGET == X3B || TARGET == OPUS
...
*/
			...
			p = lp;
			pt = lt;
/* the next two lines cause a segmentation violation on the 68000 
			lp = p->exprblock.leftp;
			lt = lp->headblock.vtype;
therefore repeat the while test from above first and then do the assigns  */
			if ( p->tag == TEXPR && 
...
p2name("");
#if TARGET == OPUS && m88k
if (reg == ARGREG && ((offset + 4) > maxargoffset))
	maxargoffset = offset + 4;
#endif
}
