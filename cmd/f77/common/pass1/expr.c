...
#if TARGET == OPUS && !m88k
		/* 
		** do what local.c does with RS here.
		** negate shift count for code generator.
		** treat lhs as a signed variable and change
		** to OPLSHIFT for table.c to work.
		*/
		if ( opcode == OPRSHIFT )
			if ( ltag != TCONST || rtag != TCONST )
			{
				if ( rtag == TCONST )
					rp->constblock.Const.ci = 
						-rp->constblock.Const.ci;
				else
					rp = mkexpr(OPNEG, rp, ENULL);
				opcode = OPLSHIFT;
			}
#endif
 ...
 * Only for VAX. OPUS cc keeps float functions as float type because of
 * IEEE.
 */
#if TARGET == OPUS
