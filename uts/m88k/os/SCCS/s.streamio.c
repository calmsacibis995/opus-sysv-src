h10823
s 03199/00000/00000
d D 1.1 90/03/06 12:28:12 root 1 0
c date and time created 90/03/06 12:28:12 by root
e
u
U
t
...
#ifdef STREAMS
/*
 * $Header: streamio.c  88/03/10 15:00:59 dtk Exp $
 * $Locker:  $
 *
...
#ident	"$Header: streamio.c  88/03/10 15:00:59 dtk Exp $"
...
struct inode *ip;
{
	...
	if (reset) {
		/*
		 * we previously found another inode for the same device,
		 * and copied the i_sptr.  The stream was waiting for open
		 * to complete.  We must reset the i_sptr, and check again.
		 */
		ip->i_sptr = 0;
		reset = 0;
	}
	...
	stp->sd_sigflags = 0;
	stp->sd_pollflags = 0;
	stp->sd_allocb  = 0;		/* -dap@mot	*/
	stp->sd_copyin  = 0;		/* -dap@mot	*/
	stp->sd_maxfrag = 0;		/* -dap@mot	*/
	stp->sd_sel     = 0;		/* -dap@mot	*/
	stp->sd_rsel    = 0;		/* -dap@mot	*/
	stp->sd_wsel    = 0;		/* -dap@mot	*/
...
struct inode *ip;
{
	...
	while (qp->q_next) {
		...
		/*
		 * need to try to run any STREAMs stuff here, since the
		 * qdetach may have caused something to be sent
		 * (see timod), and we want to avoid the above sleep,
		 * if possible, since it will run the entire time, once
		 * started. -dap@mot
		 */
		if (qready())
			runqueues();
...
/*
 * Stream select procedure.
 *
 * Returns:
 *	0	-	cannot current perform operation (read/write)
 *			steps taken to notify when able to.
...
register mblk_t *bp;
{
	...
	case M_PASSFP:
		 ...
		 */
		if (stp->sd_rsel) {
			selwakeup(stp->sd_rsel, stp->sd_sel&STR_RCOL);
			stp->sd_rsel = 0;
			stp->sd_sel &= ~STR_RCOL;
		}
	...
	case M_HANGUP:
		...
		if (stp->sd_wsel) {
			selwakeup(stp->sd_wsel, stp->sd_sel & STR_WCOL);
			stp->sd_wsel = 0;
			stp->sd_sel &= ~STR_WCOL;
		}
		return;
	...
	case M_SETOPTS:
		 ...
		 * Motorola mods.
		 *
		 *  The MAX/MIN PSZ and HI/LO WAT used to refer to the
		 *  read side queue, but that is useless.  A flag has been
		 *  added to cause the write side queue to be affected.
		 */
		if (sop->so_flags & SO_WRITE_Q) {
			if (sop->so_flags & SO_MINPSZ)
				WR(q)->q_next->q_minpsz = sop->so_minpsz;

			if (sop->so_flags & SO_MAXPSZ)
				WR(q)->q_next->q_maxpsz = sop->so_maxpsz;

			if (sop->so_flags & SO_HIWAT)
				WR(q)->q_next->q_hiwat = sop->so_hiwat;

			if (sop->so_flags & SO_LOWAT)
				WR(q)->q_next->q_lowat = sop->so_lowat;
		} else {
			if (sop->so_flags & SO_MINPSZ)
		...
		if (sop->so_flags & SO_OBEYWOFF)
			stp->sd_flag |= OBEYWOFF;

		if (sop->so_flags & SO_WOFFEACH)
			stp->sd_flag |= WOFFEACH;

		if (sop->so_flags & SO_MAXFRAG)
			stp->sd_maxfrag = sop->so_maxfrag;

		if (sop->so_flags & SO_OWNALLOC)
			stp->sd_allocb = sop->so_allocb;

		if (sop->so_flags & SO_OWNCOPYIN)
			stp->sd_copyin = sop->so_copyin;

		if (sop->so_flags & SO_OWNCOPYOUT)
			stp->sd_copyout = sop->so_copyout;
...
struct inode *ip;
{
	...
	do {
		...
		if (!(mp = strmakemsg(&mctl, &mdata, stp, 0))) return;
...
int flag;
{
	...
	/*if (is_socket(ip) && sck_ioctl(ip, cmd, arg, flag) < 0)
		return;*/
...
register flag;
{
	...
	if (is_socket(ip)) {
		/*
		 * we do not permit putmsg/getmsg on sockets
		 */
		u.u_error = EINVAL;
		return;
	}
...
register flag;
{
	...
	if (!(mp = strmakemsg(mctl, mdata, stp, flag)))
		return;
 ...
 */
mblk_t *
strmakemsg(mctl, mdata, stp, flag)
register struct strbuf *mctl;
register struct strbuf *mdata;
struct stdata *stp;
long flag;
{
	...
	alc   = stp->sd_allocb ? stp->sd_allocb : allocb;
	wroff = stp->sd_wroff;
	...
	if (mdata->len >= 0) {
		...
		do {
			register size;
			register class;
			register maxfrag = stp->sd_maxfrag;

			if (maxfrag)
				size = min(count, maxfrag) + (offlg ? 0:wroff);
			else
				size = count + (offlg ? 0 : wroff);

getbp:
			if (size < QBSIZE) {
				if (bp = (*alc)(size, pri)) goto gotbp;
			} else {
				...
				/*
				 * pass size to first call to allocator
				 * this is a kluge
				 * -dap@mot
				 */
				if (bp = (*alc)(size, pri))
				...
				/*
				 * if we failed, and OBEYWOFF is set,
				 * don't downsize request.
				 */
				if ((stp->sd_flag & OBEYWOFF) == 0) {
					if (bp = (*alc)(rbsize[--class], pri))
						goto gotbp;
			...
			/*
			 * If we are using our own allocator, and we were
			 * denied, switch to using the default allocb()
			 * and retry. -dap@mot
			 */
			if (alc != allocb) {
				alc = allocb;
...
gotbp:
			...
			/*
			 * if sd_maxfrag is set, limit size to it.
			 */
			if (maxfrag)
				size = min(size, maxfrag);

			if (size) {
				int copyin();

				/*
				 * Select the method of transfer.  If we have
				 * our own copyin routine, use it instead,
				 * if we succeeded using our own allocator.
				 * -dap@mot
				 */
				if ((*((size >= 1024
				        && alc != allocb
				        && stp->sd_copyin)
				       ? stp->sd_copyin : copyin))
				     (base, bp->b_wptr, size)) {
					u.u_error = EFAULT;
					freeb(bp);
					freemsg(mp);
			...
			/*
			 * if WOFFEACH is set, we pretend that we have not
			 * added any write offset.
			 * -dap@mot
			 */
			if (stp->sd_flag & WOFFEACH)
				offlg = 0;
		} while (count);
	}
	return(mp);
...
{
	enosys();
	if (strfake == 0)		/* XXX fake linker */
		iomove();		/* XXX fake linker */
}
strread()
{
	enosys();
