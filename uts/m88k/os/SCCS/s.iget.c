h51113
s 00663/00000/00000
d D 1.1 90/03/06 12:28:00 root 1 0
c date and time created 90/03/06 12:28:00 by root
e
u
U
t
...
#endif
{
	...
	if ((ip->i_flag & ILOCK) == 0) {
		printf("&ip = %x PA = %x\n", ip, kvtophys(ip));
		printf("i_ftype = %x, i_flag = %x\n", ip->i_ftype, ip->i_flag);
		/* fall into panic */
	}
...
iinit()
{
	(*bdevsw[bmajor(rootdev)].d_open)(rootdev, 
					FREAD|FWRITE,OTYP_LYR);
	(*bdevsw[bmajor(pipedev)].d_open)(pipedev, 
					FREAD|FWRITE,OTYP_LYR);
	srmountfun(1);
}
