h40637
s 00634/00000/00000
d D 1.1 90/03/06 12:27:57 root 1 0
c date and time created 90/03/06 12:27:57 by root
e
u
U
t
...
I 1
/*
 * $Header: flock.c  88/01/14 09:46:56 root Rel $
 * $Locker:  $
 */
 ...
 *
 * LAINFS NOTE: reclock() was renamed sys_reclock(). See fs/nfs/lockd.c
 */
#ifdef LAINFS
sys_reclock(ip, lckdat, cmd, flag, offset)
#else
		...
		case F_WRLCK:
			if ((found=blocked(*lock_list, lckdat, &insrt)) == NULL) {
				...
				if (deadflck(found)) {
					 ...
					 * sleep, as the rdwri client has the
					 * inode locked, and another client
...
/* LAINFS NOTE: cleanlocks() was renamed sys_cleanlocks(). See fs/nfs/lockd.c */
#ifdef LAINFS
sys_cleanlocks(ip,flag)
#else
cleanlocks(ip,flag)
#endif
struct inode *ip;
