h54161
s 00504/00000/00000
d D 1.1 90/03/06 12:28:15 root 1 0
c date and time created 90/03/06 12:28:15 by root
e
u
U
t
...
struct ostat *ub;
{
	...
	if (user) {
		...
		bcopy((caddr_t)&ds, (caddr_t)ub, sizeof(ds));
	}
}
...
fcntl()
{
	...
	case F_SETLK:
	case F_SETLKW:
		i = fp->f_inode->i_ftype;
		if (i != IFREG && i != IFDIR && i != IFBLK) {
			u.u_error = EINVAL;
			return;
		}
...
smount()
{
	...
	if ((uap->flags & MS_DATA) == 0) {
		uap->fsdataptr = (char *)0;
		uap->fsdatalen = 0;
	}
	(*fstypsw[fstyp].fs_mount)(bip, mp, uap->flags, uap->fsdataptr,
				   uap->fsdatalen);
#else
