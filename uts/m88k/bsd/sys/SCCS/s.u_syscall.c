h51787
s 00001/00001/00876
d D 1.2 90/03/21 09:45:42 root 2 1
c fixed accept for BCS NET
e
s 00877/00000/00000
d D 1.1 90/03/06 12:39:08 root 1 0
c date and time created 90/03/06 12:39:08 by root
e
u
U
t
...
socket()
{
	...
	if ((fp = falloc(NULL, FREAD|FWRITE)) == NULL)
		return;
	u.u_error = socreate(uap->domain, &so, uap->type, uap->protocol);
	if (u.u_error)
		goto bad;
	fp->f_inode = (struct inode *)so->so_ip;
	return;
bad:
	frelse(fp, u.u_rval1);
}
...
bind()
{
	...
	if (u.u_error)
		return;
	u.u_error = sobind((struct socket *)fptosock(fp), nam);
	m_freem(nam);
}
...
listen()
{
	...
	if (fp == 0)
		return;
	u.u_error = solisten((struct socket *)fptosock(fp), uap->backlog);
}
...
noname:
	...
	if (fp == 0)
		return;
	s = splnet();
	so = (struct socket *)fptosock(fp);
	if ((so->so_options & SO_ACCEPTCONN) == 0) {
D 2
		u.u_error = EINVAL;
E 2
I 2
		u.u_error = (so->so_type == SOCK_STREAM) ? EINVAL : EOPNOTSUPP;
E 2
		splx(s);
		return;
	}
	...
	while (so->so_qlen == 0 && so->so_error == 0) {
		...
		if (sleep((caddr_t)&so->so_timeo, PCATCH|(PZERO+1))) {
			so->so_error = EINTR;
			break;
		}
	}
...
connect()
{
	...
	if ((so->so_state & SS_NBIO) &&
	    (so->so_state & SS_ISCONNECTING)) {
		u.u_error = EAGAIN;	/* was EINPROGRESS */
		m_freem(nam);
		return;
	}
	s = splnet();
...
socketpair()
{
	...
	if (u.u_error)
		goto free;
	fp1 = falloc((struct inode *)so1->so_ip, FREAD|FWRITE);
	if (fp1 == NULL)
		goto free2;
	sv[0] = u.u_rval1;
	fp2 = falloc((struct inode *)so2->so_ip, FREAD|FWRITE);
	if (fp2 == NULL)
		goto free3;
	sv[1] = u.u_rval1;
	...
	int flags;
{
	...
	if (mp->msg_name) {
		...
		to = 0;
	if (mp->msg_accrights && mp->msg_accrightslen) {
		u.u_error =
	...
	len = auio.uio_resid;
	u.u_error =
	    sosend((struct socket *)fptosock(fp), to, &auio, flags, rights);
	...
	caddr_t namelenp, rightslenp;
{
	...
	len = auio.uio_resid;
	u.u_error =
	    soreceive((struct socket *)fptosock(fp), &from, &auio,
		flags, &rights);
	u.u_rval1 = len - auio.uio_resid;
	if (mp->msg_name) {
		len = mp->msg_namelen;
...
shutdown()
{
	...
	if (fp == 0)
		return;
	u.u_error = soshutdown((struct socket *)fptosock(fp), uap->how);
}
...
setsockopt()
{
	...
	u.u_error =
	    sosetopt((struct socket *)fptosock(fp), uap->level, uap->name, m);
}
...
getsockopt()
{
	...
	if (uap->val) {
		...
		valsize = 0;
	u.u_error =
	    sogetopt((struct socket *)fptosock(fp), uap->level, uap->name, &m);
	if (u.u_error)
		goto bad;
...
bsdpipe()
{
	...
	if (u.u_error)
		goto free;
	rf = falloc((struct inode *)rso->so_ip, FREAD);
	if (rf == NULL)
		goto free2;
	r = u.u_rval1;
	wf = falloc((struct inode *)wso->so_ip, FWRITE);
	if (wf == NULL)
		goto free3;
	u.u_rval2 = u.u_rval1;
	u.u_rval1 = r;
