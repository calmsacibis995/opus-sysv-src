h36078
s 00000/00263/01285
d D 1.2 90/05/02 09:29:56 root 2 1
c removed all debugging code.
e
s 01548/00000/00000
d D 1.1 90/03/06 12:28:40 root 1 0
c date and time created 90/03/06 12:28:40 by root
e
u
U
t
...
D 2
#define DEBUG_LOPRIQ 1	/* enable low-priority disk queue debugging */
...
D 2
	if ((bp->b_flags & B_LOPRIQ) || bp == iobp || indktab(bp)) {
		dumpbp(bp);
		dkpanic("dkstrategy");
	}
E 2
...
D 2
	if ((bp->b_flags & B_WAIT) && !(bp->b_flags & B_READ)) {
		dumpbp(bp);
		dkpanic("dkstart: B_WAIT");
	}
E 2
	bp->b_flags &= ~B_WAIT;
	splx(sect);
...
D 2
dkdbg(0x40, incompletebp);
E 2
	if ((bp = incompletebp) == NULL) {
D 2
dkdbg(0x41, dktab1.b_actf);
E 2
		if ((bp = dkcoalesce()) == NULL) {
D 2
dkdbg(0x42, dktab1.b_actf);
E 2
			return;
		}
	} else {
D 2
dkdbg(0x43, dktab1.b_actf);
E 2
		incompletebp = NULL;
	}
D 2
...
D 2
	if (!(bp == iobp && bp == dktab.b_actf) || (bp->b_flags & B_LOPRIQ)) {
		dumpbp(bp);
		dkpanic("dkintr: bad bp");
	}
E 2
	...
	if ((bp->b_rcount == 0) ||
		...
		dktab.b_active = 0;
D 2
if (dktab.b_actf && (dktab.b_actf->b_flags & B_LOPRIQ)) {
	dumpbp(dktab.b_actf);
	dkpanic("dkintr 1");
}
dkdbg(7, bp);
...
if (dp == &dktab) {
	if (bp->b_flags & B_LOPRIQ) {
		dumpbp(bp);
		dkpanic("dksort(dktab)");
	}
	dkdbg(10, bp);
} else {
	if (!(bp->b_flags & B_LOPRIQ)) {
		dumpbp(bp);
		dkpanic("dksort(dktab1)");
	}
	dkdbg(11, bp);
}
E 2
...
D 2
			checkaddr(cacheaddr(bp), bp->b_rcount);
E 2
			cp->rhits ++;
...
if (dktab.b_actf && (dktab.b_actf->b_flags & B_LOPRIQ)) {
dumpbp(bp);
dkpanic("dkcoalesce 3");
}
E 2
			}
D 2
			checkaddr(cacheaddr(nbp), nbp->b_rcount);
E 2
...
checkaddr(addr, count)
register caddr_t addr;
{
	if ((addr  < dkpage) || ((addr + count) > (dkpage + ctob(ndkpgs)))) {
		printf("bad addr = 0x%x, count = %d\n", addr, count);
		dumpbp(NULL);
		dkpanic("checkaddr");
	}
}
...
D 2
	if (bp == iobp || !(bp->b_flags & B_LOPRIQ)) {	/* currently executing */
		dumpbp(bp);
		dkpanic("dkpromote: iobp or flags");
	}
...
D 2
	if (lbp->av_forw == NULL) {
		dumpbp(bp);
		dkpanic("dkpromote: bp not found");
	}
...
register struct buf *bp;
{
	register struct buf *sbp;
	if (bp) {
		printf("bad bp=0x%x rcount=%d flags=0x%x\n",
			bp, bp->b_rcount, bp->b_flags);
		sbp = dktab.b_actf;
		while (sbp) {
			if (sbp == bp)
				break;
		...
		if (sbp)
			...
			if (sbp)
				printf("buffer found in dktab1\n");
			else
				printf("buffer not in any table\n");
		}
	} else {
		printf("bp=NULL\n");
	}

	printf("&dkbuf = 0x%x dkpage = 0x%x &dktab = 0x%x, &dktab1 = 0x%x\n", 
		&dkbuf, dkpage, &dktab, &dktab1);
	printf("iobp = 0x%x incompletebp = 0x%x\n", iobp, incompletebp);
}

#if DEBUG_LOPRIQ
#define NDBG 64	/* power of 2! */
static struct xx {
	unsigned short cnt, seq;
	struct buf *bps, *act;
	ushort flags[2];
} xx[NDBG];
static xxp;
...
struct buf *bp;
{
	...
	p->bps = bp;
	p->act = dktab.b_actf;
	p->flags[0] = bp->b_flags;
	if (dktab.b_actf)
		p->flags[1] = dktab.b_actf->b_flags;
	else
		p->flags[1] = 0;
	if (dktab.b_actf && (dktab.b_actf->b_flags & B_LOPRIQ)) {
		printf("seq=%d bp=%x\n", val, bp);
		dkpanic("dkdbg: dktab.b_actf had LOPRIQ");
	}
	if (dktab.b_actf && (dktab.b_actl->b_flags & B_LOPRIQ)) {
		printf("seq=%d bp=%x\n", val, bp);
		dkpanic("dkdbg: dktab.b_actl had LOPRIQ");
	}
	splx(s);
}
#endif
...
D 2
#if DEBUG_LOPRIQ
	printf("\ndkpanic: p=%x xx=%x ", &xx[xxp&(NDBG-1)], xx);
	printf ("&dktab=%x &dktab1=%x\n", &dktab, &dktab1);
#endif
E 2
	panic(fmt, ARGS);
}
