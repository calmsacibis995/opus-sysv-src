h60432
s 01097/00000/00000
d D 1.1 90/03/06 12:29:02 root 1 0
c date and time created 90/03/06 12:29:02 by root
e
u
U
t
...
sxtioctl(dev, cmd, arg, mode)
{
	...
	case TC_PX_SETATTRF:
		...
		if (CHAN(dev) == lp->controllingtty) {
			/* must perform action on real tty */
			/* insure that line disps remain correct */
			if (copyin(arg, &cbs, sizeof cbs)) {
				u.u_error = EFAULT;
				break;
			}
			c_line = cbs.c_line;
			cbs.c_line = lp->line->t_line;
			if (copyout(&cbs, arg, sizeof cbs)) {
				u.u_error = EFAULT;
				break;
			}
			(*cdevsw[major(lp->dev)].d_ioctl)
				(minor(lp->dev), TC_PX_SETATTR, arg, mode);
			/*  now restore user buffer */
			cbs.c_line = c_line;
			if (copyout(&cbs, arg, sizeof cbs)) {
				u.u_error = EFAULT;
	...
	case TC_PX_BREAK:
		ttiocom(&lp->chans[CHAN(dev)].tty, cmd, arg, mode);
		if (CHAN(dev) == lp->controllingtty) {
			if ((lp->line->t_brktime = arg) == 0)
				(*lp->line->t_proc)(lp->line, T_BREAK);
			else {
				if (cmd == TC_PX_BREAK) {
					lp->line->t_brktime = timerround(arg);
					(*lp->line->t_proc)(lp->line, T_BREAK);
				}
			}
		}
