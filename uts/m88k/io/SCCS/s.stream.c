h18505
s 01560/00000/00000
d D 1.1 90/03/06 12:29:00 root 1 0
c date and time created 90/03/06 12:29:00 by root
e
u
U
t
...
I 1
/*
 * $Header: stream.c  88/04/15 10:34:57 tom Exp $
 * $Locker:  $
 *
 ...
 */
#ident	"$Header: stream.c  88/04/15 10:34:57 tom Exp $"
...
/*
 * Extra space is added to each block in some classes to allow for
 * write offset.  80 bytes is currently sufficient for our uses.
 */
#ifndef EXTRA_512
#	define EXTRA_512	80
#	define EXTRA_1K		80
...
ushort	bsize[]  = { 4, 16, 64, 100, 200, 300, 600, 1200, 2400, 4800, 9600, 19200 };
...
gotdp:
	...
	databp->db_ref = 1;
	/*
	 * clear shared indication (should always be zero, but just in case)
	 * -dap@mot
	 */
	databp->db_shared = 0;
...
long arg;
{
	...
	if ((pri == BPRI_HI) && (*(&v.v_nblk32768 + (NCLASS - 1 - class)) == 0)) 
		return(0);
...
#ifdef SEX
	/*
	 * If we have a shared datab, hand it to SEX to return it to
	 * the common pool.
	 * -dap@mot
	 */
	if (bp->b_datap->db_shared) {
		sx_freeb(bp);
		return;
	}
#endif
...
register mblk_t *bp;
{
	...
	nbp->b_datap = bp->b_datap;

#ifdef SEX
	/*
	 * use interlocked access to increment shared datab reference counts
	 * -dap@mot
	 */
	if (bp->b_datap->db_shared)
		sx_incb(&bp->b_datap->db_ref);
	else
#endif
		++bp->b_datap->db_ref;
...
#define str_aligned(X)	(((uint)(X) & (sizeof(short) - 1)) == 0)
...
strinit()
{
	...
	size =  btoc(v.v_nblk32768*(32768+EXTRA_32K)
                   + v.v_nblk16384*(16384+EXTRA_16K)
                   + v.v_nblk8192*(8192+EXTRA_8K)
                   + v.v_nblk4096*(4096+EXTRA_4K)
		   + v.v_nblk2048*(2048+EXTRA_2K)
		   + v.v_nblk1024*(1024+EXTRA_1K)
		   + v.v_nblk512 *( 512+EXTRA_512)
		   + v.v_nblk256 *  256
		   + v.v_nblk128 *  128
		   + v.v_nblk64  *   64
		   + v.v_nblk16  *   16
	...
	if (!(base = (unsigned char *)getcpages(size, NOSLEEP))) {
		goto alcfail;
        }
	 ...
	 */
	databp = &dblock[0];
	for (class = NCLASS-1, vnp = &v.v_nblk32768; class >=0; class--, vnp++) {
...
queuerun()
{
	...
	do {
		if (strbcflag) {
			strbcflag = 0;
			for (i=NCLASS-1, vnp = &v.v_nblk32768; i>=0; i--, vnp++){
				dbp = &dballoc[i];
