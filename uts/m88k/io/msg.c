...
msgctl()
{
	...
	case IPC_SET:
		...
		microtime((struct timeval *) &qp->msg_ctime);
		break;
	case IPC_STAT:
...
{
/* begin MINX FIX */
	if ((pmp == NULL && mp != qp->msg_first) ||
		(pmp && pmp->msg_next != mp)) {
		/* bogus/stale pmp. recompute pmp */
		if (mp == qp->msg_first)
			pmp = NULL;
		else {
			for (pmp = qp->msg_first; pmp ; pmp = pmp->msg_next)
				if (pmp->msg_next == mp)
					break;
			if (pmp == NULL)
				cmn_err(CE_PANIC,"can't find previous message");
		}
	}
/* end MINX FIX */
...
findmsg:
	...
	if (smp) {
		...
		microtime((struct timeval *) &qp->msg_rtime);
		curpri = PMSG;
		msgfree(qp, spmp, smp);
		goto msgrcv_out;
	}
...
getres:
	...
	microtime((struct timeval *) &qp->msg_stime);
	mp->msg_next = NULL;
	mp->msg_type = type;
	mp->msg_ts = cnt;
