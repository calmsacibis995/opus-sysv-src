h54854
s 00004/00000/01020
d D 1.2 90/06/22 00:42:13 root 2 1
c worked around the FsINOPB macro
e
s 01020/00000/00000
d D 1.1 90/03/06 12:43:58 root 1 0
c date and time created 90/03/06 12:43:58 by root
e
u
U
t
...
s5rmount(iflg)
{
	...
	if (iflg) {
		time = rtcread();
		if(time < fp->s_time)  /* if no rtc or lost time setting */
			wtodc(fp->s_time); /* use super block time */
		/* 
...
int  ufstyp;
{
	...
	extern short s5fstyp;
	dev_t dev;
	int   mshift; /* Shift value for physical to logical blocks */
	...
	sp->f_fstyp = s5fstyp;
	sp->f_frsize = 0;
	sp->f_blocks = fp->s_fsize << mshift;
	sp->f_bfree = fp->s_tfree << mshift;
I 2
#if 1
	sp->f_files = (fp->s_isize - 2) * (sp->f_bsize/sizeof(struct dinode));
#else
E 2
...
realign(srcep, destp, fmt, reverse)
char *srcep;	/* pointer to source structure */
char *destp;	/* pointer to destination structure */
char *fmt;
int reverse;	/* if non-zero, convert new to old */
{
	/* fmt describes the structure whose alignment is being changed.
	 * Each character in fmt corresponds to a member of the structure.
	 * Valid characters in fmt are:
	 *	c	- character
	 *	s	- short
	 *	i	- integer
	 *	l	- long
	 ...
	 *	d	- double
	 *
	 * These characters may be preceded by a count, to indicate an
	 * array of that type.
	 *
	 * For example,
	 *
	 ...
	 *	};
	 *
	 * would be represented by fmt equal to "l4cs100d".
	 *
	 */
				/* char, short, int, long, float, double */
	static char fmtchars[] = "csilfd";
	static char m68kalignment[] =	{ 1, 2, 2, 2, 2, 2 };
	static char m88kalignment[] =	{ 1, 2, 4, 4, 4, 8 };
	static char sizes[] =		{ 1, 2, 4, 4, 4, 8 };
	static char decimalchars[] =	"0123456789";
	...
	if (reverse) {
		srcealignment = m88kalignment;
		destalignment = m68kalignment;
	} else {
		srcealignment = m68kalignment;
		destalignment = m88kalignment;
	}

	count = 0;
	for ( ; *fmt; fmt++) {
		for (p=decimalchars; *p && *p != *fmt; p++)
			;
		if (*p) {
			count = count * 10 + p - decimalchars;
			continue;
		}
		for (p=fmtchars; *p && *p != *fmt; p++)
			;
		if (*p) {
			index = p - fmtchars;
			salign = srcealignment[index];
			dalign = destalignment[index];
			size = sizes[index];
			if (count == 0)
				count = 1;
			for ( ; count > 0; count--) {
				srcepad = (int)srcep % salign;
				if (srcepad)
					srcepad = salign - srcepad;
				destpad = (int)destp % dalign;
				if (destpad)
					destpad = dalign - destpad;
			...
			cmn_err(CE_PANIC, "realign: bad format char '%c'\n",
				*fmt);
		}
	}
}
...
register struct filsys	*fp;
{
	...
	if (fp->s_magic == FsALTMAGIC) {
		cmn_err(CE_NOTE, "\nConverting root filesystem to current superblock format\n");
		tmp = *fp;		/* structure assignment */

		realign(&tmp, fp, "sls50ls100sccccl4sls6c6c",
						FsALTMAGIC > FsMAGIC);
		realign(&tmp.s_state, &fp->s_state, "lll",
						FsALTMAGIC > FsMAGIC);
		fp->s_magic = FsMAGIC;
	}
}
