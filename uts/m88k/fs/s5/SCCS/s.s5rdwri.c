...
register struct inode *ip;
{
	...
	case IFDIR:
		...
		if (type == IFIFO) {
			/*
			 * check for write selectors, and enable.
			 */
			if (s5ip->s5i_wsel) {
...
register struct inode *ip;
{
	...
	case IFDIR:
		...
		if (type == IFIFO) {
			/*
			 * check for read selectors, and enable.
			 */
			if (s5ip->s5i_rsel) {
			...
			if (u.u_error == 0 && usave != 0) {
				...
				if ((u.u_fmode&FNONB) && (ip->i_size==PIPSIZ))
					return;
				goto floop;
			}
		}
...
int mode;
{
	...
	bf.l_len = u.u_count;
	i = (u.u_fmode & (FNDELAY|FNONB)) ? INOFLCK : SLPFLCK|INOFLCK;
