h21021
s 00018/00000/00143
d D 1.2 90/04/21 09:38:00 root 2 1
c call sewakeup on pipe closings
e
s 00143/00000/00000
d D 1.1 90/03/06 12:43:56 root 1 0
c date and time created 90/03/06 12:43:56 by root
e
u
U
t
	...
	register mode;
{
	...
	if (mode & FWRITE) {
		if (mode & (FNONB|FNDELAY) && s5ip->s5i_frcnt == 0) {
			u.u_error = ENXIO;
			return;
		}
		if (s5ip->s5i_fwcnt++ == 0)
	...
	if (mode & FREAD) {
		while (s5ip->s5i_fwcnt == 0) {
			if (mode & (FNONB|FNDELAY) || ip->i_size) {
				return;
			}
...
I 2
			/*
			 * check for write selectors.
			 */
			if (s5ip->s5i_wsel) {
				selwakeup(s5ip->s5i_wsel
...
I 2
			/*
			 * check for read selectors.
			 */
			if (s5ip->s5i_rsel) {
				selwakeup(s5ip->s5i_rsel
