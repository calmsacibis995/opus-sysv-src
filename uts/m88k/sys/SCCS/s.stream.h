h42185
s 00012/00012/00519
d D 1.4 90/05/05 06:49:04 root 4 3
c removed -dap@mot
e
s 00001/00001/00530
d D 1.3 90/05/05 06:46:09 root 3 2
c removed Motorola reference
e
s 00000/00000/00531
d D 1.2 90/05/05 06:44:29 root 2 1
c removed mototola references
e
s 00531/00000/00000
d D 1.1 90/03/06 12:30:23 root 1 0
c date and time created 90/03/06 12:30:23 by root
e
u
U
t
...
/*
 * $Header: stream.h  88/01/15 13:44:44 dtk Exp $
 * $Locker:  $
 *
...
struct stdata {
	...
	struct msgb *(*sd_allocb)();	/* special allocator for datab's */
	int	(*sd_copyin)();		/* special copyin		 */
	int	(*sd_copyout)();	/* special copyout		 */
	int	sd_maxfrag;		/* maximum fragment		 */
	struct	proc *sd_rsel;		/* process selecting for read	 */
	struct	proc *sd_wsel;		/* process selecting for write	 */
	unsigned char sd_sel;		/* select flags			 */
};

/*
 * sd_sel definitions
 */
#define STR_RCOL	0x01		/* read collision		 */
#define STR_WCOL	0x02		/* writ collision		 */
...
/*
D 3
 * local flags for Motorola use.
E 3
I 3
 * local flags for opus use.
E 3
 */
#define OBEYWOFF 	020000000000	/* obey write offset		*/
#define OWNALLOC	010000000000	/* use own datab allocator	*/
#define WOFFEACH	004000000000	/* need write offset for each	*/
...
D 4
	unsigned char	db_shared;	/* Streams Exec hooks. -dap@mot	*/
E 4
I 4
	unsigned char	db_shared;	/* Streams Exec hooks. */
E 4
};
...
D 4
	mblk_t *(*so_allocb)();	/* special allocator -dap@mot	*/
	int (*so_copyin)();	/* special copyin    -dap@mot	*/
	int (*so_copyout)();	/* special copyout   -dap@mot	*/
	short so_maxfrag;	/* maximum fragment  -dap@mot	*/
E 4
I 4
	mblk_t *(*so_allocb)();	/* special allocator */
	int (*so_copyin)();	/* special copyin    */
	int (*so_copyout)();	/* special copyout   */
	short so_maxfrag;	/* maximum fragment  */
E 4
};

/* flags for stream options set message */

#define SO_ALL	     017777	/* set all options */
...
D 4
#define SO_OBEYWOFF    0100	/* obey write offset   -dap@mot	*/
#define SO_OWNALLOC    0200	/* use our own allocb  -dap@mot	*/
#define SO_OWNCOPYIN   0400	/* use our own copyin  -dap@mot	*/
#define SO_OWNCOPYOUT 01000	/* use our own copyout -dap@mot	*/
#define SO_WOFFEACH   02000	/* each blk needs woff -dap@mot	*/
#define SO_MAXFRAG    04000	/* maximum fragment sz -dap@mot	*/
#define SO_WRITE_Q   010000	/* relative to write q -dap@mot	*/
E 4
I 4
#define SO_OBEYWOFF    0100	/* obey write offset   */
#define SO_OWNALLOC    0200	/* use our own allocb  */
#define SO_OWNCOPYIN   0400	/* use our own copyin  */
#define SO_OWNCOPYOUT 01000	/* use our own copyout */
#define SO_WOFFEACH   02000	/* each blk needs woff */
#define SO_MAXFRAG    04000	/* maximum fragment sz */
#define SO_WRITE_Q   010000	/* relative to write q */
E 4
 ...
 */
#define NCLASS          12		/* number of block classes */
#define QBSIZE		65		/* min size for block allocation retries */
#define MAXBSIZE	32768		/* max block size */
