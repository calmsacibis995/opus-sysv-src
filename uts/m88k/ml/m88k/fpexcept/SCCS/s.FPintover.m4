h56256
s 00361/00000/00000
d D 1.1 90/03/06 12:50:40 root 1 0
c date and time created 90/03/06 12:50:40 by root
e
u
U
t
...
TRNC:        clr    r1,r1,2<14>          ;clear rounding mode bits,
                                         ;instruction is TRNC
             br.n   checksize            ;branch to check size
             set    r1,r1,1<14>          ;make rounding mode round towards zero

NINT:        clr    r1,r1,2<14>          ;make rounding mode round to nearest
...
;being the rounding mode, and bits 3, 2, and 1 are the guard, round, and 
;sticky bits.
...
nostickyp:   extu   r11,r1,2<14>         ;shift rounding mode to 2 LSB's
...
checkdoubn:  cmp    r11,r10,29           ;compare to 30, but exp. off by 1
             ...
             bcnd   ne0,r10,overflw      ;one of the bits is a 1, so overflow
             bb0    30,r1,possround      ;rounding mode is either round near or
...
possround:   extu   r12,r8,1<20>         ;get guard bit
             ...
             set    r12,r12,1<0>         ;set sticky bit
nostickyn:   bb1    31,r1,negative       ;rounding mode is negative
...
;the rounding mode, and bits 3, 2, and 1 are the guard, round, and sticky
...
conversiondn: extu   r12,r8,1<22>       ;get LSB for integer with exp. = 30
             ...
             set    r12,r12,1<0>        ;set sticky bit
nostkyn:     extu   r11,r1,2<14>        ;shift rounding mode to 2 LSB's
             mak    r11,r11,2<3>        ;set up field for branch table
             or     r12,r11,r12         ;set up field for branch table
             lda    r12,r0[r12]         ;scale r12
...
overflw:     bb0.n  oper,r3,nohandler   ;do not go to user handler routine
...
nohandler:   bb0.n  sign,r7,return      ;if positive then return from subroutine
             set    r6,r6,31<0>         ;set result to largest positive integer
             or.c   r6,r0,r6            ;negate r6,giving largest negative int.

return:      ;ld     r1,r31,XRETADDR     ;load return address from memory
	ld	r1,r31,0
	add	r31,r31,8
