...
;First check for an 2 user handler.  If there is not one, then
;branch to the routine to make a denormalized number.  Before branching
;to the 2 user handler, add 192 to a single precision exponent
;and 1536 to a double precision exponent.

_FPunderflow:
                ;st    r1,r31,108 ;save return address
	        bb0 7,r12,denorm ;jump to default procedure
                bb1.n 10,r12,doubleprec ;double precision destination
                set   r2,r2,1<2>  ;set 2 flag 10 8
...
;the difference between a denormalized exponent and an 2 exponent and
;shifting the mantissa by that amount.  A one may need to be subtracted from 
;the LSB if a one was added during rounding.
;r9 is used to contain the guard, round, sticky, and an inaccuracy bit 10
...
denorm:         bb1.n 10,r12,double ;denorm for double
...
;and quicker to make a special case.  Examples of this are if the  
;amount is only 1 or 2, or all the mantissa is shifted off, or all the
;mantissa is shifted off and it is still shifting, or, 10 the case of 
;doubles, if the  amount is around the boundary of MANTLO and MANTHI.
...
nounrounds:     ; ld    r8,r0,zeroexp  ;load r8 with -127
		...
		ld	r8,r8,lo16(zeroexp)
                ...
                bb1   7,r6,threesing ;br to code that handles shifts of >=3
                cmp   r6,r7,2      ;check to see if r7 contains 2
                bb1   2,r6,twosing ;br to code that handles shifts of 2
one:            rot   r9,r9,0<1>   ;rotate roundoff register once, this places
                                   ;guard 10 round and round 10 sticky
...
guardclr1s:     extu  r11,r11,31<1> ; mantissa right 1
...
roundclr2s:     bb0   1,r11,guardclr2s ;do not set guard bit if LSB + 1 = 0
                set   r9,r9,1<2>   ;set guard bit
guardclr2s:     extu  r11,r11,30<2>  ; mantissa right 2
                br.n  round        ;round result
                mak   r9,r9,3<0>   ;clear bits lost during rotation
...
noguard3s:      cmp   r6,r7,23     ;check if # of shifts is <=23
                bb1   4,r6,s24    ;branch to see if shifts = 24
                sub   r6,r7,2      ;get number of bits to check for sticky
                mak   r6,r6,5<5>   ; width into width field
...
nostky23:       or    r8,r0,34     ;start code to get new mantissa plus two
                ...
                subu  r8,r8,r7     
                mak   r8,r8,5<5>   ; field width into second five bits
                extu  r6,r6,5<5>   ; previous shifted -2 into offset field
...
nogrd3s:        br.n  round        ;round mantissa
                extu  r11,r11,30<2>  ; off remaining two bits

s24:            cmp   r6,r7,24     ;check to see if # of shifts is 24
                bb1   4,r6,s25    ;branch to see if shifts = 25
...
s25:            cmp   r6,r7,25     ;check to see if # of shifts is 25
                bb1   4,r6,s26    ;branch to execute for shifts => 26
...
nostky25:       set   r9,r9,1<1>   ;set new round bit,this is hidden bit
                clr   r9,r9,1<2>   ;clear guard bit since nothing shifted 10
...
nounroundd:     ; ld    r8,r0,zeroexp+4 ;load r8 with -1023
		...
		ld	r8,r8,lo16(zeroexp+4)
                sub   r7,r8,r6     ;find difference between two exponents,
                                   ;this amount is the  amount
                cmp   r6,r7,3      ;check to see if r7 contains 3 or more
                bb1   7,r6,threedoub ;br to code that handles shifts of >=3
                cmp   r6,r7,2      ;check to see if r7 contains 2
                bb1   2,r6,twodoub ;br to code that handles shifts of 2
...
nosticky1d:     bb0   0,r11,guardclr1d ;do not set new guard bit if old LSB = 0
                set   r9,r9,1<2>   ;set new guard bit
guardclr1d:     extu  r11,r11,31<1> ; lower mantissa over 1
                mak   r6,r5,1<31>  ; off low bit of high mantissa
                or    r11,r6,r11   ;load high bit onto lower mantissa
                extu  r5,r5,20<1>  ; right once upper 20 bits of mantissa
...
roundclr2d:     bb0   1,r11,guardclr2d ;do not set guard bit if old LSB + 1 = 0
                set   r9,r9,1<2>   ;set new guard bit
guardclr2d:     extu  r11,r11,30<2> ; lower mantissa over 2
                mak   r6,r5,2<30>  ; off low bits of high mantissa
                or    r11,r6,r11   ;load high bit onto lower mantissa
                extu  r5,r5,19<2>  ; right twice upper 19 bits of mantissa
                br.n  round        ;round mantissa and assemble result
                mak   r9,r9,3<0>   ;clear bits lost during rotation
...
noguard3d:      cmp   r6,r7,32     ;do I need to work with a 1 or 2 word mant.
                ...
                bb1   4,r6,d33    ;jump to code that handles 2 word mantissas
                sub   r6,r7,2      ;get number of bits to check for sticky
                mak   r6,r6,5<5>   ; width into width field
                mak   r8,r11,r6    ;mask off shifted bits -2
                ff1   r8,r8        ;see if r8 has any ones
...
nornd32d:       bb0   1,r11,nogrd32d ;do not set new guard bit
                set   r9,r9,1<2>   ;set new guard bit
nogrd32d:       extu  r11,r11,30<2> ; off remaining two bits
                mak   r6,r7,5<5>   ; field width into second 5 bits, if the
		...
		or    r5,r0,r0         ;all of the bits to be extracted under
				       ;the regular method
regular33:      mak   r6,r7,5<0>   ;place lower 5 bits of  into r6
                mak   r8,r8,5<5>   ; r8 into width field
...
d33:            cmp   r6,r7,33     ;is the number of bits to be shifted is 33?
                bb1   4,r6,d34    ;check to see if # of bits is 34
...
nornd33:        bb0   0,r5,nogrd33 ;do not set guard bit if bit is not a 1
                set   r9,r9,1<2>   ;set new guard bit
nogrd33:        extu  r11,r5,31<1> ; high bits into low word
                br.n  round        ;round and assemble result
                or    r5,r0,r0     ;clear high word

d34:            cmp   r6,r7,34     ;is the number of bits to be shifted 34?
                bb1   4,r6,d35    ;check to see if # of bits is >= 35
...
nornd34:        bb0   1,r5,nogrd34 ;do not set guard bit if bit is not a 1
                set   r9,r9,1<2>   ;set new guard bit
nogrd34:        extu  r11,r5,30<2> ; high bits into low word
                br.n  round        ;round and assemble result
                or    r5,r0,r0     ;clear high word

d35:            cmp   r6,r7,52     ;see if # of shifts is 35 <= X <= 52
                bb1   4,r6,d53    ;check to see if # of shifts is 52
...
nostky35:       or    r8,r0,32     ;look at what does not get shifted off plus
                                   ...
                                   ;was adjusted so that it did not 
                                   ;new round or new sticky 10 shifted off bits
                subu  r8,r8,r7     ;complement width 
                mak   r8,r8,5<5>   ; width into width field
...
nornd35:        bb0   1,r11,nogrd35 ;do not set new guard bit
                set   r9,r9,1<2>   ;set new guard bit
nogrd35:        extu  r11,r11,30<2> ; off remaining guard and round bits
                br.n  round         ;round and assemble result
                or    r5,r0,r0      ;clear high word

d53:            cmp   r6,r7,53     ;check to see if # of shifts is 53
                bb1   4,r6,d54    ;branch to see if shifts = 54
...
d54:            cmp   r6,r7,54     ;check to see if # of shifts is 54
                bb1   4,r6,d55    ;branch to execute for shifts =>55
...
;rounding 31.  The third bit is either the LSB of the mantissa or the
;31 bit, depending on the rounding 31.  The three LSB's are the guard,
...
round:          ff1   r8,r9         ;see if there is denormalization loss
                ...
                extu  r6,r10,2<29>   ;extract rounding 31
                bb1.n 30,r10,signext ;use 31 bit instead of LSB
                mak   r6,r6,2<4>    ; over rounding 31
                extu  r7,r11,1<0>   ;extract LSB
                br.n  grs           ;skip 31 extraction
                mak   r7,r7,1<3>    ; over LSB
signext:        extu  r7,r10,1<31>  ;extract 31 bit
                mak   r7,r7,1<3>    ; 31 bit over
grs:            or    r6,r6,r7
                or    r6,r6,r9      ;or 10 guard, round, and sticky
		...
		add	r6,r6,lo16(roundtable)
                ...
                set   r9,r9,1<3>    ;set 0 flag 10 r9
...
addone:         or    r6,r0,1      ;load a 1 into r6 so that add.co can be used
                ...
                bb0.n 10,r12,noaddone ;single result,forget carry
                set   r9,r9,1<4>   ;indicate that a 1 has been added
                add.ci r5,r5,r0    ;propagate carry into high word


;Branch to 0 user handler if there is one.

noaddone:       bb1.n 5,r12,modformdef ;branch to modify form for user 
                                             ;handler
                or    r2,r2,5      ;set 0 and 2 flags
...
;so simply assemble the 31 and the new mantissa.

assemble:       bb1   10,r12,doubassem ;assemble double result
                bb0   31,r10,exassems ;exit assemble if 31 is zero
                set   r11,r11,1<31>  ;make result negative
exassems:       br    return       ;return from subroutine

doubassem:      bb0.n 31,r10,signclr ;do not set 31 10 r10
		or    r10,r5,r0    ;load high word from r5 into r10
                set   r10,r10,1<31> ;high word with 31 loaded
signclr:        br    return       ;return from subroutine
                

;modfordef modifies the result of denormalization to the input format of
;the 0 user handler.  This input format is the same format that 
...
modformdef:     clr   r12,r12,12<20> ;clear result exponent,IMPCR complete
                ...
                bb0.n 4,r9,newaddone ;do not set new addone 10 MANTHI
                or    r10,r5,r10     ;or 10 new grs field
                set   r10,r10,1<25>  ;set new addone
newaddone:
                clr   r10,r10,21<0>  ;clear upper bits of old mantissa
                bb1 10,r12,moddefd ;branch to handle double precision
...
return:         ;ld    r1,r31,108 ;load return address
                ...
                data
zeroexp:        word  0x00000f81   ;-127 10 decimal for lowest 12 bits
                word  0x00000c01   ;-1023 10 decimal for lowest 12 bits
